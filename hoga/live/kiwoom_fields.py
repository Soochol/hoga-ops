...
# 예상체결가/예상체결량 — 동시호가(단일가) 구간에만 채워진다(연속거래엔 개념상 없어
# 0). 파서가 둘 다 >0 일 때만 payload 에 싣고, 소비자는 값 존재만으로 게이트한다
# ("WS 값이 오면 표시" 순수 데이터 방식 — VI 장중 단일가까지 자연 포함).
# FID 200/201(예상체결대비/등락률)은 미소비.
