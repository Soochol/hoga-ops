...
# 예상체결가/예상체결량 — 동시호가(단일가) 구간에만 채워진다. 연속거래 중엔 0으로
# 오므로(장전 08:30–09:00·장마감 15:20–15:30·VI 단일가에만 유의미) 소비자는
# "값>0"으로 게이트해 평시엔 숨긴다. FID 200/201(예상체결대비/등락률)은 미소비.
# ⚠ 미검증: 레포에 동시호가 0D 캡처가 없어 "연속거래 중 =0" 가정은 실장 검증 대상
# ([[kiwoom-ws-fid-catalog]] — 값 의미는 산술검산 확정, 연속장 거동만 미채록).
