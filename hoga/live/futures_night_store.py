...
읽기는 `FuturesQuotesRuntime._recall_night` 이 쓴다 — 낮·마감에 카드가 "직전 야간"
선택지를 갖는 경로다. **야간 세션 중에는 읽지 않는다**: 오늘 야간이 도는데 지난 밤을
곁들이면 무음 카드에서 둘이 섞인다.

`latest_session_day(before=오늘)` 이 그 선택의 핵심이다. 오늘 밤 파일은 야간이
시작되면 즉시 쌓이기 시작하는데, 낮에 "어젯밤" 을 물었을 때 그것을 집으면 **오늘
저녁 값을 어젯밤이라고 부르게 된다**.
