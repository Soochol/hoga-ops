...
class TickDownsampler:
    ...
    def __init__(self) -> None:
        # 키 = **(bare code, venue)**. venue 를 빼면 KRX·NXT 의 같은 종목이 한
        # `_CodeState` 를 공유해 `last_ob` 가 나중 도착분으로 덮이고, 10초 flush 가
        # 뒤섞인 값 하나만 쓴다(ADR-0140 §2). `split_venue` 가 bare code 를 주므로
    ...
    def set_active_codes(self, codes: set[str]) -> None:
        ...
        # 멤버십은 **종목 단위**다 — 코드가 Live Set 을 떠나면 그 종목의 모든
        # venue 상태를 함께 버린다.
    ...
    def flush(
        ...
        """윈도 마감 — (코드,venue)별 [ob?, broker?, fill] 반환. 흐름 합은 리셋,
