...
- **30초 폴, 동일 값이면 미기록.** 주식과 같은 근거로 60 → 30 을 따라갔지만
  **파생은 갱신 상한을 아직 못 쟀다**: 2026-08-10 실측에서 표본 89개가 **전부** 값
  변화였다(중복 저장 0). 매 폴마다 바뀐다는 것은 60초가 상한보다 느리다는 뜻일 뿐
  얼마나 자주 바뀌는지는 말해 주지 않는다. 30초에서 중복이 나오기 시작하면 그때가
  실제 주기이고, 여전히 0 이면 더 조일 여지가 남아 있다는 신호다.
