...
class LiveBuffer:
    ...
    async def get_series(self, code: str) -> dict:
        ...
        async with self._lock:
            ...
            xp_buf = self._buf.get((code, SnapshotKind.EXPECTED.value))
            ...
            expected = tuple(xp_buf) if xp_buf else ()
        ...
        return {
            ...
            # 예상체결(0H) — `snapshots` 와 **다른 배열이어야 한다**. 사다리가 없는
            # payload 라 섞으면 소비자가 빈 호가창을 그린다(0E 와 같은 이유).
            "expected": [_strip_t_only(e) for e in expected],
        }
