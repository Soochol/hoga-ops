...
def check_disk_state(
    ...
    if per_source:
        ...
        # 우선순위 = `sources.ORDERFLOW_LADDER` 와 동기 유지(import 는 순환이라 불가 —
        # sources 가 disk_state 를 import 한다). 2026-08-07: 사다리가 키움 우선으로
        # 바뀌고 `kis_api` 가 제거되면서 이 튜플도 같이 옮겼다.
        winning = next(
            (per_source[src] for src in ("kiwoom_live", "hogaplay")
