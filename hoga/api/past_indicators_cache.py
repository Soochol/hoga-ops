...
KIND_VERSIONS: dict[str, int] = {
    ...
    #     온디맨드로 자연히 채워진다.
    # 13: traded_bar_peaks/traded_bar_max_peaks(봉별 최대 체결 벽) 추가 — 최대벽 강도
    #     pane 의 **봉별 모드** 입력. 구 캐시엔 이 필드가 없고 pydantic 이 **빈 리스트로
    #     조용히 채우므로**(`depth` 8 범프와 같은 함정) 범프하지 않으면 그 모드를 켠
