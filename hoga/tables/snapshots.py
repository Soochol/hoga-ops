...
def _book_indicator_eligible_sql(
    ...
    *,
    ...
    ⚠ **경계를 인자로 받는 이 형태가 계약이다.** 프론트 술어는 한동안 개장 하한을
    ``isAfterRegularOpen``(09:00 KST)으로 함수 안에 박아 두었고, 그 비대칭이 정확히
    "NXT/통합 선택 시 프리·애프터마켓 호가 지표가 통째로 빈다"는 버그였다(캔들 축은
    venue 별 확장창인데 호가만 정규장). 프론트도 필수 인자로 바뀌어 지금은 양쪽이
    실제로 같은 정의다 — **어느 쪽이든 시각을 다시 함수 안에 넣지 말 것.**

    **venue 축 (해소됨 · 2026-08-10)**: 한동안 여기 들어오는 open/close 가 날짜 메타의
    ``regular_session_*``(09:00–15:30)뿐이라 과거일 경로가 venue 를 타지 않았다. 그때
    적어 둔 TODO 가 "디스크에 venue 축 캡처가 0건이라 지금은 무해하지만, 생기는 시점에
    메타에 venue 별 경계를 싣고 호출부가 넘겨야 한다" 였는데 — 2026-08-06 부터 실제로
    쌓이기 시작했고, **예고한 재발이 그대로 일어났다**(8/7 NXT/UN 호가 스냅샷의 49.1%가
    프리·애프터마켓이라 통째로 배제. 지문은 "포인트는 08:00~20:00 전 구간 오는데 값만 0").
    지금은 promote 가 ``indicator_session_open/close_ms`` 를 venue 별로 싣고
    (``_VENUE_INDICATOR_SESSION_MS``), 호출부가 ``indicator_session_bounds`` 로 읽어
    넘긴다. 그 키가 없는 meta(hogaplay·구형)는 ``regular_session_*`` 로 폴백하므로 KRX
    경로는 무변경이다. **경계를 여기 다시 박지 말 것** — 이 문단이 그래서 남아 있다.
...
class AskPeakDualRow:
    ...
    **동일분 터치** ask events (그 벽이 관측된 1분 안에서 체결이 그 가격을 친
    ...
    #: `_peak_bar_max_sequence`). 최대벽 강도 pane 의 **봉별 모드** 입력이고,
    #: `traded_record_*`(누적 계단)와 같은 데이터의 다른 축이다. ⚠ 이쪽은 **봉
    #: 의존**이라 rep/cont 둘 다 1분 해상도가 정본이다 — 굵은 봉은 창 안 1분 값들의
...
def _peak_bar_max_sequence(df: pl.DataFrame) -> tuple[AskPeakCandidateRow, ...]:
    ...
    - dedup 키는 `bucket_id` **하나**다(가격이 아니다). 같은 봉의 여러 가격 중 가장
      큰 것 하나가 그 봉의 값이고, 같은 가격이 여러 봉에서 각각 최대여도 봉마다
      자기 값을 갖는다 — `_peak_touched_distinct`(가격당 rank-1)를 입력으로 쓰면
      두 번째 봉이 통째로 사라진다.
    - **cap 이 없다.** 봉별 계열은 잘린 구간이 "체결된 벽이 없던 봉"과 구별되지
      않으므로 앞뒤 어느 쪽을 잘라도 거짓말이 된다 — 기록 시퀀스가 꼬리를 잘라도
      `traded_peaks` 가 보정하는 것과 사정이 다르다. 페이로드는 대신 라우트 게이트가
