...
def _book_indicator_eligible_sql(
    ...
    *,
    ...
    TODO(venue 축): 여기 들어오는 open/close 는 아직 날짜 메타의 ``regular_session_*``
    (09:00–15:30)뿐이라 **과거일 경로는 venue 를 타지 않는다**. 지금은 무해하다 —
    과거 캡처가 전부 hogaplay(KRX 전용, ADR-0003)라 애초에 NXT 시간대 호가가 없고,
    ``kiwoom_live/<venue>/`` 축 캡처는 디스크에 0건이다(2026-08-07 확인). venue 축
    과거 캡처가 생기는 시점에 **메타에 venue 별 세션 경계를 싣고 호출부가 그걸 넘겨야**
    한다. 안 그러면 오늘 라이브에서 고친 것과 똑같은 증상이 과거일에서 재발한다.
