 ...
 *
 * ⚠ `/study` 캔버스로의 **그룹 드롭**은 여기를 타지 않는다. 그건 앱 안 네비게이션이고
 * 목적지가 그 페이지 자신이라, 드로어가 `/study?view=` 리터럴을 직접 쓴다 — 그 경로는
 * 페이지와 함께 죽는다. 이름을 `study…` 에서 `savedView…` 로 바꾼 이유가 그것이다:
 * 이 모듈이 가리키는 곳은 이제 페이지가 아니라 **저장뷰라는 도메인**이다.
 *
