 ...
 *
 * ⚠ **포커스 창이 아닌 봉으로 쿼리를 낼 때 이 훅을 쓰면 안 된다** — 한때 그렇게 했다가
 * 사고가 났다. 이 훅은 **포커스 창의 봉**으로 지표를 푸는데, 쿼리의 `bucket_ms` 가 다른
 * 봉이면 "분봉 버킷 + D 프로필 지표" 라는 잡종 키가 나가고, 활성 전환에서 키가 안 맞아
 * 같은 구간을 플래그만 바꿔 한 번 더 받는다 — 요청을 줄이려다 늘린다. 그 사고의 무대는
