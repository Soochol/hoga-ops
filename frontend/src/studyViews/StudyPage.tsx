...
export function StudyPage() {
  ...
  /**
   * 저장뷰를 **활성 그룹에** 연다(ADR-0152 — 드로어의 `openStudyView` 와 같은 규칙).
   *
    ...
    () => workspaceWindows
      ...
      .map((w) => {
        ...
        return {
          windowId: w.id,
          group: w.group,
          // 저장뷰도 **창에서** 온다(ADR-0152) — 그룹마다 다를 수 있으므로 훅 인자로
  ...
  // 보존 대상은 **어느 그룹이든 지금 보고 있는 종목 전부**다(ADR-0152). 활성 그룹
  ...
  /** 메모 저장 — **어느 저장뷰인지 인자로 받는다**(ADR-0152: 메모 창도 그룹에 딸린다).
  ...
  /**
   * 페이지가 빈 상태로 가는 조건은 **어느 그룹에도 저장뷰가 없을 때**다(ADR-0152).
   *
  ...
  for (const spec of chartWindowSpecs) {
    ...
    const model = studyActiveViewModel({
      // 폴백도 **이 창의 그룹** 저장뷰다(ADR-0152) — 활성 뷰로 떨어뜨리면 그룹 2 창이
          ...
          >
            ...
            >
              <StudyWorkspaceCanvas
                // 데이터·메모 창은 **자기 그룹의 포커스 차트 창** 번들을 먹는다(ADR-0152).
                saveFor={(windowId) => {
