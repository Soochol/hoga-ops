...
const {
  ...
  capturedWindowSpecs: {
    current: [] as Array<{
      windowId: string;
      // ADR-0152: 저장뷰가 창 스펙 안으로 들어왔다 — 그룹마다 다를 수 있다.
...
vi.mock('./useStudyReferenceBundle', () => ({
  useStudyReferenceBundles: (
    ...
    // 저장뷰는 ADR-0152 로 **창 스펙 안**에 있다 — 창마다 그룹이 다를 수 있다.
    const save = w.save;
...
function bundle(): RangeBundle {
  return {
    ...
    quote_ratio: {
      bucket_ms: 300_000,
      points: [{ t: 1_000, bid_total: 100, ask_total: 90, bid_max: 0, ask_max: 0, imb_max_bid: 0, imb_max_ask: 0, band_pct: 0, tick: 0 }],
    },
...
function openStudyView(view: { viewId: string; code: string; label: string; name: string }) {
  act(() => {
    // ADR-0152: 저장뷰는 **활성 그룹**에 꽂힌다. 시드 창이 전부 그룹 1 이다.
...
beforeEach(() => {
  ...
  useStudyWorkspaceStore.setState({
    ...
    // 그룹→저장뷰도 함께 비운다(ADR-0152) — 여기가 활성 뷰의 집이다.
    groupViews: {},
  });
});
...
/**
 * 링크 그룹 (ADR-0152) — 한 페이지에서 저장뷰를 **여럿** 본다.
 *
 ...
 */
describe('StudyPage — 링크 그룹 (ADR-0152)', () => {
 ...
 */
describe('StudyPage — 그룹에 차트 창이 없을 때 (ADR-0152)', () => {
