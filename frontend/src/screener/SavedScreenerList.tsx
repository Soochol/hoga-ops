  ...
  return (
    <div ref={rootRef} className="flex h-full flex-col gap-sm min-h-0 overflow-auto p-md">
      <div className="flex items-center gap-1.5">
        <span className="text-xs font-semibold uppercase text-fg-dim">저장한 조건검색</span>
        ...
        {visibleSaves.map((s) => {
          ...
          return (
            <ListRow key={s.id} role="button" tabIndex={0}
              ...
              {isAnchor && dirty && !isRenaming && <span className="shrink-0 text-badge text-fg-dim">수정됨</span>}
              {!isRenaming && (
