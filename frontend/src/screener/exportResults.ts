...
/** 내보내기는 조회 스냅샷과 클릭 순간의 마지막 수신 시세를 code로 결합한다.
 *  편집 중인 조건이나 표시 모드를 입력으로 받지 않아 조회 근거가 바뀌지 않는다. */
export function resultsCsv(
  scan: PanelScan, codes: readonly string[], liveRows: readonly ScreenerRowLive[], exportedAtMs: number,
): string {
  const snapshots = new Map(scan.rows.map((r) => [r.code, r]));
...
export function downloadResultsCsv(scan: PanelScan, codes: readonly string[], liveRows: readonly ScreenerRowLive[]): void {
  const now = Date.now();
  const url = URL.createObjectURL(new Blob([resultsCsv(scan, codes, liveRows, now)], { type: 'text/csv;charset=utf-8' }));
