/** 저장된 스크리너 조건 목록 의 쿼리 키 — 소비처가 **둘**이라 단일 출처로 둔다: ① 이 창의 저장
 *  mutation ② 다른 창·다른 브라우저의 변경이 WS 로 도착했을 때(api/eventStream).
 *
 *  **api 모듈이 아니라 여기(키 전용 모듈)에 두는 이유**: `api/savedScreeners` 는 테스트가
...
export const SCREENER_SAVES_KEY = ['screener-saves'] as const;
