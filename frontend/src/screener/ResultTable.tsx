...
const COLS = 'grid-cols-[4.5rem_minmax(10rem,1fr)_4rem_8.5rem_6rem_6rem]';
const SELECTABLE_COLS = 'grid-cols-[1.5rem_4.5rem_minmax(10rem,1fr)_4rem_8.5rem_6rem_6rem]';
  ...
  return (
    <DataTableRow role="button" tabIndex={0} aria-label={`${r.name} ${r.code} 호가창 열기`}
      ...
      {occurrenceController && !!r.occurrences?.length && <OccurrenceDetails code={r.code} name={r.name}
        occurrences={r.occurrences} historyMatches={r.history_matches} controller={occurrenceController} />}
    </DataTableRow>
  );
}

export function ResultTable({ rows, onActivate, sortMode = 'default', onSortChange, embedded = false, depthValues, depthSides, selection, quoteMode = 'live', occurrenceController }: Props) {
  ...
  return (
    <DataTableShell
      scrollRef={shellRef}
      minWidth={selection ? '760px' : '720px'}
