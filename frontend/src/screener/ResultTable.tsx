...
function DepthRenewalBadge({ v, sides }: { v: DepthPeakValue; sides: DepthSides }) {
  ...
  const title = shown
    ...
    .join(' / ');
  return (
    <span className="inline-flex items-center gap-1.5 font-data text-badge tabular-nums text-fg-dim"
      title={title}>
      {shown.map((r) => (
...
function DepthBadge({ v, sides }: { v: DepthPeakValue; sides: DepthSides }) {
  ...
  const title = rows
    ...
    .join(' / ');
  return (
    <span className="inline-flex items-center gap-1.5 font-data text-badge tabular-nums text-fg-dim" title={title}>
      {rows.map((r) => (
        <span key={r.label} className="inline-flex items-center gap-1">
...
function SortHeader({ field, label, sortLabel = label, align, sortMode = 'default', onSortChange }: {
  ...
  return (
    ...
    >
      ...
      <span className="font-data text-badge" aria-hidden="true">{arrow}</span>
    </button>
  );
}
  ...
  return (
    <DataTableRow role="button" tabIndex={0} aria-label={`${r.name} ${r.code} 호가창 열기`}
      ...
      {r.expected_price != null ? (
        ...
        >
          <span className="mr-0.5 text-badge text-fg-dim">예</span>
