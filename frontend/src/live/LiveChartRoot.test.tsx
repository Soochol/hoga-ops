...
describe('LiveChartRoot lazy fetch trigger', () => {
  ...
  it('fires extendHistoricalRange with yesterday YYYYMMDD when axis includes a prior segment', () => {
    // Override useLiveBundle mock to return yesterday+today segments so the
    // axis maps small virtual values to yesterday's real time.
    ...
    // Virtual seconds within yesterday's segment (virtualStart=0, so any
    // small positive value lands inside yesterday's real session).
    const fromSec = 1000;  // 1000s × 1000 = 1_000_000 virtual ms → ~16 min into yesterday
    act(() => {
      handlers.forEach((h) => h({ from: fromSec, to: fromSec + 600 }));
      // Trailing debounce is 150ms (eng review C3).
      vi.advanceTimersByTime(200);
    });

    const next = useLivePageStore.getState().historicalFromDate;
    expect(next).toBe('20260526');
  });
