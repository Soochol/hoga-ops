...
const COPY = {
  ...
  congestion: {
    // 원인 명사구는 표면 공통 사전에서 온다. 예전엔 여기가 '시세 서버 혼잡'(상태)이라
    // 스크리너의 '호출 한도 초과'(원인)와 같은 kind 를 다르게 불렀다 — 본문이 이미
    // 상태를 서술하므로 제목은 원인을 말한다(ADR-0143 §5-A).
    title: WARNING_CAUSE.rate_limit,
