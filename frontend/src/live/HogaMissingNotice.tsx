...
import { CHART_TIME_AXIS_PX } from './chartPaneLayout';
 ...
 * 날짜에 머무는 내내 참이다).
 *
 * 소스 배지와 크롬·자리를 공유하므로 `stacked` 로 한 칸 밀어 올릴 수 있다.
 */
const noticeStyle: CSSProperties = {
  ...
  // 올라온다 — PaneLegendOverlay 가 같은 이유로 zIndex 4 를 쓴다.
  zIndex: 4,
};
...
export function HogaMissingNotice({
  ...
  if (!text) return null;
  const base = timeAxisVisible ? `${CHART_TIME_AXIS_PX}px + var(--space-2xs)` : 'var(--space-xs)';
