...
export function LiveSidebar({ code, live, bundle = null, todayKst = '', programTrade = null }: Props) {
  ...
  // 매물대 카드를 숨기면 무거운 매물대 훅(오늘 증분 + 커서 컷오프)을 끈다(접기 제거
  // 2026-07-15 이후 숨김으로 대체). SSE 는 LivePage 소유라 불변(ADR-0040) — 카드
  // 언마운트로 렌더만 멈추고, 이 게이트로 재계산 자체를 멈춘다.
  const volumeCardHidden = useLiveLayoutStore((s) => s.rightCardHidden.volumeDistribution ?? false);
  ...
  const todayVolumeDistribution = useLiveTodayVolumeDistribution({
    enabled: volumeDistributionEnabled && !!activeBundle && !volumeCardHidden,
    stockCode,
    todayKst,
  ...
  const cutoffVolumeDistribution = useVolumeDistributionCutoffProfile({
    enabled: volumeDistributionEnabled && volumeDistributionHoverCutoffEnabled && isSpot && !volumeCardHidden,
    code: stockCode,
