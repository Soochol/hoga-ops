...
function BookWindow({ win, code }: { win: WorkspaceWindow; code: string }) {
  ...
  // 여기서 미리 풀지 않는다 — 해석 지점이 둘로 갈리면 한쪽만 고쳐지는 게 이
  // 파일이 이미 겪은 실패다(useLiveCursor 의 VenueParam 주석).
...
function BrokerWindow({ win, code }: { win: WorkspaceWindow; code: string }) {
  ...
  //
  // 넘기는 값은 **선택값**이고, 세 경로가 각자 `useEffectiveVenue` 로 종목별 유효
  // venue 를 해석한다 — WS 꼬리는 liveSeries.ts 의 filterByVenueTag, 나머지 둘은
  // URL·캐시 키. 예전엔 훅 두 개가 선택값을 그대로 백엔드에 보내서, NXT 미상장
  // 종목 + UN 선택이면 창이 통째로 비었다(#1211 후속, 근거·실측은 useLiveCursor 의
  // VenueParam 주석).
  const venue = useLiveVenueStore((s) => s.venue);
  const live = useLiveSeries(code, venue);
  const { cursorMs, timeframe: cursorTimeframe } = useGroupCursor(win.group);
