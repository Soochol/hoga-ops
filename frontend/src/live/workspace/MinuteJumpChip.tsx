...
export function MinuteJumpChip({
  ...
  // ⚠ **`landed` 에 "이동했습니다" 를 쓰지 않는다.** 그 상태는 착지와 **중단**을
  // 함께 담는다(사용자가 백필을 기다리는 동안 그 창을 만지면 그 seq 는 포기된다 —
  // `useTimeframeJump` 의 래치 절). 중단된 경우 창은 움직인 적이 없으므로 "이동했다"
  // 는 거짓이고, 그건 이 리포가 「안 되는 것만 말한다」로 금지하는 종류다. 두 경우에
  // 모두 참인 것은 **대상이 무엇인가**뿐이라 그것만 적는다.
