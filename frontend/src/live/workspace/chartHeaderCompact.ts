...
/**
 * 2026-09-10 시스템 Chrome, 1.0× 밀도에서 헤더 복제본의 max-content 폭 실측.
 * chart-legend-toggle.spec.ts에서 분봉 240분 / 일봉, 세 접힘 단계를 재현한다.
 *                          전체       라벨 접힘    봉까지 접힘
 * 240분 (레전드 포함)       473.08      309.22       177.98
 * 240분 (레전드 제외 대조)  449.08      285.22       153.98
 * 일봉 (레전드 포함)        638.50      348.13       188.44
 * 일봉 (레전드 제외 대조)   614.50      324.13       164.44
 *
 * 일봉에는 분봉 이동·조회 날짜·패턴 영역 버튼이 추가된다. 실측값을 올림한다.
 * 마지막 단계도 최소 창 폭보다 넓으므로 ChartWindow는 버튼 묶음을 줄바꿈한다.
 * 컨테이너 폭으로만 접힘을 결정하므로 줄바꿈이 접힘 판정을 되돌리지 않는다.
 */
 ...
 * 검증한 모양) 필요 폭을 더 낮춘다.
 *
 * 임계 = `actionsFolded` 요구폭(310, 위 표) + 여유 5 = **315**.
 *
