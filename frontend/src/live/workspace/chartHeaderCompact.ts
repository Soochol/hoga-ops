 ...
 *
 * **실측 2026-08-14** (`/browse` · 다이얼 1.0× · 액션 **5버튼**(관심 하트 추가) ·
 * #905 복제 절차: 헤더를 `width:max-content` 컨테이너에 복제해 자연폭을 읽음.
 * 단계는 창 리사이즈 핸들을 실제로 드래그한 뒤 **reload 해서** 재현했다 —
 * 마운트 시 `push(clientWidth)` 경로가 단계를 확정하므로 흉내가 아니다):
 *
 * | 단계          | 30분 라벨 | 1분 라벨 | 하트 제외(30분) | 종전 상수 |
 * |---------------|-----------|----------|-----------------|-----------|
 * | full          | **412**   | 405      | 382             | 384 임계  |
 * | actionsFolded | **257**   | 250      | 235             | 240 임계  |
 * | bothFolded    | **148**   | 141      | 148 (동일)      | —         |
 *
 * **하트를 뺀 열이 종전 실측 382 / 235 와 정확히 일치한다** — 절차가 재현됐다는 뜻이고,
 * 재측정할 때도 이 대조를 남길 것(그전에는 두 밀도를 함께 재서 같은 확인을 했다).
 * 하트 추가분은 full 에서 +30(패딩 `px-2`), 접힌 단계에서 +22(`COMPACT_PADDING_INLINE`).
 *
 * `bothFolded` 에 하트가 없는 것은 실수가 아니다 — 그 단계에서는 하트를 **렌더하지
 * 않는다**(ChartWindow). 하트를 포함하면 170px 이 필요한데 `MIN_W`(160px) 창의
 * 컨테이너는 158px 이라 12px 넘치고, 넘친 만큼 「수집」이 무성 잘렸다(실측 확인).
 *
...
export const LIVE_HEADER_NEED = {
  /** 다 펴진 상태: 봉 그룹 + 라벨 단 액션 4개 + 하트. */
  full: 412,
  /** 1단계: 액션 라벨 접힘(아이콘 4개) + 하트 + 봉 그룹. */
  actionsFolded: 257,
  /** 2단계: 봉 드롭다운 + 아이콘 4개. 하트는 이 단계에서 빠진다. */
  bothFolded: 148,
} as const;
 ...
 * 창 rect 를 sessionStorage 로 바꿔 reload 해서 단계를 재현):
 *
 * | 단계          | 분봉 창(대조) | 캘린더 창 | Δ   |
 * |---------------|---------------|-----------|-----|
 * | full          | **405**       | 475       | +70 |
 * | actionsFolded | **250**       | 272       | +22 |
 * | bothFolded    | **141**       | 135       | 0   |
 *
 * **대조 열이 위 표의 「1분 라벨」 열(405 / 250 / 141)과 정확히 일치한다** — 절차가
 * 재현됐다는 증거다. 재측정할 때도 같은 대조를 남길 것.
 *
 * Δ 를 30분 라벨 기준 `LIVE_HEADER_NEED` 에 더한 값이 아래 상수다. `bothFolded` 가
 ...
 * 검증한 모양) 필요 폭을 더 낮춘다.
 *
 * 임계 = `actionsFolded` 요구폭(257, 위 표) + 여유 5 = **262**.
 *
