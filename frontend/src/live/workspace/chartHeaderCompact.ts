 ...
 *
 *  **실측 2026-08-07** (`/browse` · 다이얼 1.0× · 액션 4버튼 · #905 복제 절차:
 *  헤더를 `width:max-content` 컨테이너에 복제해 자연폭을 읽음):
 *    1분 라벨 **375px** · 30분 라벨 **382px** → 임계 = 382 + 여유 2 = **384**.
 *  같은 절차를 옛 다이얼(root 18px)로 돌리면 414.25 / 422.25 가 나와 종전 실측
 *  415 를 0.75px 오차로 재현한다 — 절차가 검증됐다는 뜻이므로, 재측정할 때도
 *  **두 밀도를 함께 재서 이 재현을 확인**하는 것을 권한다.
...
export const HEADER_LABEL_MIN_WIDTH_PX = 384;
 ...
 * 검증한 모양) 필요 폭을 더 낮춘다.
 *
 * **실측 2026-08-07** (1단계 접힘 = 액션 아이콘만 + 봉 그룹 유지, 위와 같은 절차):
 *   1.0× → 1분 **228px** · 30분 **235px** → 임계 = 235 + 여유 5 = **240**.
 *   1.125× → 246.25 / 254.25.
 *
 ...
 */
export const HEADER_TIMEFRAME_FOLD_WIDTH_PX = 240;
