 ...
 * ## 왜 `null` 이 있나
 *
 * `breakdown` 은 **종목 경로만** 채운다. 그리고 옛 응답을 든 웜 캐시에서는 키가
 * 아예 없을 수 있다 — 캐시는 메모리 전용이라(ADR-0048) 프로세스 재기동이 유일한
 * 무효화이고, 그때까지는 같은 URL 이 분해 없는 행을 돌려준다. 그 구간을 0 으로
 * 그리면 "그날 개인은 순매수 0" 이라는 **거짓말**이 되므로 `null` 로 구분하고,
 * 화면은 그 칸을 값이 아니라 공백으로 그린다.
 */
