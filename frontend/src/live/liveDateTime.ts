 ...
 *  개시라 **이 리포가 이미 쓰는 경계**이며(`nxtPhaseLabel`, 백엔드 `_quote_phase`
 ...
 *
 * 백엔드 `LiveBuffer._last_ob` 는 날짜를 넘겨 살아남는다 — 그건 되돌릴 결정이 아니라
 * 의도다(`last_ob_store` docstring: "재시작이 동작을 바꾸지 않는다"). 이 술어는 그
 * 위에 **표시 정책만** 얹는다: 저장은 그대로 두고 화면에서만 내린다. 그래서 새 탭과
 * 밤새 켜 둔 탭이 같은 화면이 된다 — 후자는 프레임을 클라이언트 ref 가 붙들고 있어
 * (`useLiveSeries` 의 `lastSeenObRef`) 백엔드를 고쳐도 원리적으로 닿지 않는다.
 *
 * ## 막는 방향
 *
 ...
 *
 *  **2 → 1 (2026-08-21).** 실사용 팬 실측에서 **재취득 비용이 0** 으로 확인됐기
 *  때문이다: 자른 구간으로 되돌아가면 `coverage_gap` 확장이 뜨지만 네트워크 요청은
 *  0 건이었다(청크 캐시 `['range', …]` 가 30분 살아 있어 즉시 복원). 여유가 사 주는
 *  것이 "재취득 회피" 인데 그 대가가 없으므로, 회수 쪽에 더 준다. */
export const CONTRACT_RETAIN_STEPS = 1;

/** 남은 창이 뷰포트 좌단에서 **이 스텝보다 더 과거**일 때만 자른다(발동 임계).
 *
 *  히스테리시스의 다른 한쪽. `CONTRACT_RETAIN_STEPS` 와의 간격이 자를 때마다 실제로
 *  회수되는 양이자, 자른 뒤 재요청까지의 완충이다. 둘을 붙이면 뷰포트가 조금만
 *  흔들려도 자르기↔재요청이 왕복한다 — #582 wide-range 사고와 같은 모양이다.
 *
 *  **6 → 3 (2026-08-21).** 6스텝은 1분봉에서 **30거래일(약 6주)** 이라, 성능 감사가
 *  실측한 아픈 케이스인 **한 달 창(21거래일, 콜드 11.7초)** 이 임계 아래로 빠져
 *  영영 안 잡혔다. 3스텝(15거래일)이면 한 달 창이 잡힌다.
 *
 *  낮출 근거는 실측이다 — 6/3 시절 팬 테스트에서 `contract` 1회 후 30초간 추가
 *  이벤트 0, 진동 없음을 확인했다. 간격은 2스텝(10거래일)으로 **유지**된다.
 *
 *  ⚠ **단위를 거래일로 바꾸지 말 것 — 2026-08-24 에 시도했다가 되돌렸다.**
 *  「1분봉 기준으로 튜닝된 값이 10m 에서 10배(150거래일)로 작동한다」가 바꾸려던 이유인데,
 *  그 관찰은 맞지만 **결론이 틀렸다**. `stepCandlesEstimate` 가 분봉 스텝을 tf 무관
 *  **1950봉**으로 맞춰 둔다(1m 5거래일 × 390 = 10m 50거래일 × 39 = 60m 300거래일 × 6.5).
 *  이 두 임계가 지키는 것은 메모리 상주량, 즉 **봉 수**이므로 스텝이 곧 그 불변 단위다.
 *  거래일로 고정하면 tf 마다 상주량이 갈린다 — 「10m 의 150거래일」은 버그가 아니라
 *  봉 수 불변이 작동한 모습이고, 10m 한 달 창(~820봉)은 애초에 회수할 것이 없다.
 *
 *  게다가 **확장은 스텝 단위로 남아 있어** 단위가 갈리는 순간 위 3-상수 불변식이
 *  뒤집힌다: 10m 확장 1스텝(50거래일) > trigger(15거래일)라 갓 확장한 창이 곧바로
 *  잘렸다. 브라우저 실측에서 `viewport_backfill_extend` 1초 뒤 `viewport_backfill_contract`
 *  가 되돌리는 진동이 났고(`from: 20260609 → contractTo: 20260729`), 14회 팬에도 창이
 *  안 넓어졌다 — #582 wide-range 사고와 같은 모양이다. 그 변경은 vitest 6129 ·
 *  typecheck · build · e2e 를 **전부 통과**했다. `planViewportContraction` 테이블의
 *  「갓 확장한 창은 즉시 잘리지 않는다」가 그 구멍을 막는다. */
export const CONTRACT_TRIGGER_STEPS = 3;
...
export function planViewportContraction(
  ...
  // 캔들이 사라졌다 재로드). 벤더 모드 분봉 캔들은 병합 캐시가 깊이를 따로 보존해
  // 이 문제가 없다. ⚠ **디스크 모드(hogaplay/우회) 분봉은 그 전제가 거짓이다** —
  // 캔들이 range 창에서 직접 오므로 축소가 캔들을 실제로 잘라낸다. 그 커밋의 뷰포트는
  // `useViewportBackfill` 의 좌측 트림 재투영(2c)이 지킨다(2026-08-25 실측: 보정 전엔
  // 축 원점 재매핑에 lwc 재앵커가 제멋대로 착지해 수 주 미래로 순간이동했다).
  // D/W/M 게이트는 배선(useViewportBackfill)이 아니라 여기 커널에 둔다 —
