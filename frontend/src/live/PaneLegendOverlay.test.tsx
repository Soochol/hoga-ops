...
describe('PaneLegendOverlay — 동기화 크로스헤어의 값 행 연동', () => {
  ...
  it('flag provider 도 동기화 봉의 시각을 받는다 — 일봉→분봉 방향', () => {
    ...
    registerFlagLegendValues(null, 'ask-peak', 'main', provider);
    try {
      render(
        <PaneLegendOverlay
          chart={makeChart([120])}
      ...
      unregisterFlagLegendValues(null, 'ask-peak', 'main', provider);
    }
  });
});
...
// 2026-07-22 에 밀집도를 이유로 flag 행을 전부 숨겼고, 2026-08-22 에 당일 최대벽 둘만
// 되살렸다. **이 PR 이 그 결정을 뒤집는다** — 레전드 행이 지표의 유일한 조작 표면(눈·✕)이
// 된 이상, 켜 놓은 지표가 레전드에 없으면 끄러 모달을 열어야 한다. 밀집도는 이제 행 숨김이
// 아니라 가로 축약(container query)으로 푼다. cells pane 화이트리스트는 **그대로**다 —
// 그쪽은 값 판독면 이중화가 근거라 이 결정과 무관하다.
describe('PaneLegendOverlay — flag 행 부활(전 종류 표시)', () => {
  beforeEach(resetStore);
  afterEach(cleanup);
  ...
  it('꺼진 flag 지표는 여전히 행이 없다 — 부활은 화이트리스트 해제이지 무조건 표시가 아니다', () => {
    useLivePageStore.setState({ movingAverages: [], tradeVolumePocEnabled: false });
    render(<PaneLegendOverlay chart={minutePanes()} timeframe="1m" paneToggles={toggles} />);
  ...
  it('부활한 flag 의 provider 값도 함께 표시된다', () => {
...
describe('PaneLegendOverlay — 당일 최대벽 flag 행(2026-08-22)', () => {
  ...
  it('보이는 영역 상위 3개를 순위 라벨과 함께 표시한다', () => {
    useLivePageStore.setState({ movingAverages: [], askPeakEnabled: true });
    registerFlagLegendValues(null, 'ask-peak', 'main', topThree);
    try {
      render(<PaneLegendOverlay chart={minutePanes()} timeframe="1m" paneToggles={toggles} />);
      ...
      unregisterFlagLegendValues(null, 'ask-peak', 'main', topThree);
    }
  });

  // 눈은 그대로 비파괴 숨김이고, ✕ 의 뜻만 "끄기" → **"삭제"** 로 바뀌었다.
  // flat 싱글턴은 뺄 배열이 없으므로 삭제 = 그 지표가 소유한 필드를 전부 공장값으로.
  it('눈은 숨김을, ✕ 는 삭제(공장값 리셋)를 디스패치한다', () => {
    ...
    useLivePageStore.setState({ bidPeakColor: '#123456', bidPeakLineWidth: 4 });
    fireEvent.click(screen.getByRole('button', { name: '당일 매수 최대벽 삭제' }));
    expect(useLivePageStore.getState().bidPeakEnabled).toBe(false);
    // 끄기와 달리 **손댄 값도 함께 되돌아간다** — 이게 삭제와 끄기의 차이다.
    expect(useLivePageStore.getState().bidPeakColor)
      .toBe(FACTORY_INDICATOR_SETTINGS.bidPeakColor);
    expect(useLivePageStore.getState().bidPeakLineWidth)
      .toBe(FACTORY_INDICATOR_SETTINGS.bidPeakLineWidth);
    // 되돌릴 기회를 남긴다(칩 ✕ 와 같은 규율).
    expect(useLivePageStore.getState().indicatorUndoToast?.label).toBe('당일 매수 최대벽 삭제됨');
  });

  it('flag ✕ 의 실행취소가 삭제 전 설정을 되살린다', () => {
    useLivePageStore.setState({
      movingAverages: [],
      bidPeakEnabled: true,
      bidPeakColor: '#123456',
    ...
    expect(useLivePageStore.getState().bidPeakColor).toBe('#123456');
    expect(useLivePageStore.getState().bidPeakLineWidth).toBe(4);
  });

  it('보이는 영역에 벽이 없어(셀 0개) 값이 비어도 행과 아이콘은 남는다', () => {
    useLivePageStore.setState({ movingAverages: [], askPeakEnabled: true });
    const empty: FlagLegendValueProvider = () => [];
    registerFlagLegendValues(null, 'ask-peak', 'main', empty);
    try {
      render(<PaneLegendOverlay chart={minutePanes()} timeframe="1m" paneToggles={toggles} />);
      expect(screen.getByText('당일 매도 최대벽')).toBeInTheDocument();
      expect(
        screen.getByRole('button', { name: '당일 매도 최대벽 삭제' }),
      ).toBeInTheDocument();
    } finally {
...
describe('PaneLegendOverlay — pane 병합/분리 (칩 메뉴·드래그)', () => {
  ...
  it('칩 메뉴 y축 모드 — 공유 전환 시 오버라이드 기록 + 축 배지 즉시 소멸', () => {
    useLivePageStore.getState().setPaneGroups(
      mergePaneIntoGroup(useLivePageStore.getState().paneGroups, 'ratio', 'volume'),
    );
    render(<PaneLegendOverlay chart={minutePanes()} timeframe="1m" paneToggles={toggles} />);
    // 비화이트리스트 그룹의 기본은 격리 → 대표 칩에 축 배지가 있고, 메뉴는 공유·왼쪽 축을 권한다.
    expect(
      within(screen.getByTestId('pane-chip-volume')).getByLabelText('오른쪽 축 눈금 소유'),
    ).toBeInTheDocument();
    clickChip('ratio');
    expect(screen.getByTestId('pane-menu-axis-shared')).toBeInTheDocument();
    expect(screen.getByTestId('pane-menu-axis-left')).toBeInTheDocument();
    expect(screen.queryByTestId('pane-menu-axis-isolated')).toBeNull(); // 현재 모드는 항목 없음
    fireEvent.click(screen.getByTestId('pane-menu-axis-shared'));
    // 오버라이드가 구성 키(정렬)로 기록되고…
    expect(useLivePageStore.getState().paneAxisMode)
      .toEqual({ [paneGroupKey(['volume', 'ratio'])]: 'shared' });
    // …공유 상태에선 축 배지가 사라지며(한 축이므로 소유 표시가 무의미), 메뉴는 「분리」로.
    expect(
      within(screen.getByTestId('pane-chip-volume')).queryByLabelText('오른쪽 축 눈금 소유'),
    ).toBeNull();
    clickChip('ratio');
    expect(screen.getByTestId('pane-menu-axis-isolated').textContent).toContain('y축 분리');
    expect(screen.queryByTestId('pane-menu-axis-shared')).toBeNull();
  });

  it("칩 메뉴 'left' 모드 — 둘째 멤버 이름이 항목에 실리고 좌축 배지가 붙는다", () => {
    useLivePageStore.getState().setPaneGroups(
      mergePaneIntoGroup(useLivePageStore.getState().paneGroups, 'ratio', 'volume'),
    );
    render(<PaneLegendOverlay chart={minutePanes()} timeframe="1m" paneToggles={toggles} />);
    clickChip('ratio');
    const leftItem = screen.getByTestId('pane-menu-axis-left');
    expect(leftItem.textContent).toContain('『호가비』 왼쪽 축에 표시');
    fireEvent.click(leftItem);
    expect(useLivePageStore.getState().paneAxisMode)
      .toEqual({ [paneGroupKey(['volume', 'ratio'])]: 'left' });
    // 대표 칩 = 우축 배지, 둘째 칩 = 좌축 배지.
    expect(
      within(screen.getByTestId('pane-chip-volume')).getByLabelText('오른쪽 축 눈금 소유'),
    ).toBeInTheDocument();
    expect(
      within(screen.getByTestId('pane-chip-ratio')).getByLabelText('왼쪽 축 눈금 소유'),
    ).toBeInTheDocument();
    // 메뉴엔 「왼쪽 축 숨기기」(격리 복귀)가 나온다.
    clickChip('ratio');
    expect(screen.getByTestId('pane-menu-axis-isolated').textContent).toContain('왼쪽 축 숨기기');
  });

  it('그룹을 해체하면 그 구성의 축 모드 오버라이드도 걷힌다 (스테일 소멸)', () => {
    ...
    useLivePageStore.getState().setPaneAxisMode(['volume', 'ratio'], 'shared');
    // 분리 → 매칭 그룹이 사라져 오버라이드도 정규화에서 걷힌다.
    useLivePageStore.getState().setPaneGroups(
      extractPaneToBoundary(merged, 'ratio', 2),
    );
    expect(useLivePageStore.getState().paneAxisMode).toEqual({});
  });
