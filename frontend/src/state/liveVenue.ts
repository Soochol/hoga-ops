...
export const LIVE_VENUE_HELP =
  'KRX는 정규장(09:00–15:30)과 2026년 9월 14일부터 애프터마켓(16:00–20:00)을 봅니다. '
  + '애프터마켓 대상 종목은 제한됩니다. NXT는 프리·애프터마켓을 포함한 08:00–20:00을 봅니다. '
  + '통합은 거래소가 병합해 내보내는 단일 호가라, 두 시장을 화면에서 더한 것이 아닙니다. '
  // 탭 전역이라는 사실을 여기서 알린다 — 탭마다 다를 것으로 기대하고 두 탭을
  // 띄운 사용자는, 알리지 않으면 한쪽이 "저절로 바뀌었다"고 읽는다.
  //
  // 예외를 같이 적는 이유: 복기 탭도 "열려 있는 탭" 이라 예외를 빼면 이 문장이
  // 거짓이 된다(ADR-0144). 안 적으면 `/live` 에서 NXT 를 고른 사용자가 옆 복기
  // 탭이 안 따라온 것을 고장으로 읽는다.
  + '선택은 열려 있는 모든 탭에 함께 적용됩니다 — 복기뷰(/study)만 예외로 항상 KRX입니다.';
