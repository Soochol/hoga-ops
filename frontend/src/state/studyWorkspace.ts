 ...
 * 저장뷰를 창마다 다른 봉으로 나란히 보는 것이었고, ADR-0155 로 **창마다 다른
...
/**
 * 그룹이 가리키는 저장뷰 — `/live` `GroupSymbol` 의 `/study` 짝(ADR-0155).
 *
...
export interface StudyWorkspaceWindow {
  ...
  /** 링크 그룹 = **저장뷰** SSOT (ADR-0155). 창은 저장뷰를 직접 들지 않는다. */
  group: GroupId;
 ...
 *
 * `group` 을 주면 **그 그룹 안에서만** 고른다(ADR-0155). 데이터 창이 "내 그룹의 어느
...
function readStorage(): Persisted {
   ...
   *
   * 여기는 반대다. `study.activeView.v1` 은 ADR-0155 로 **쓰는 사람이 사라진** 키라
