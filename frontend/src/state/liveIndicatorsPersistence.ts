...
export type PersistedIndicators = {
   ...
   *
   *  ⚠ `bar` 는 **세 계열 전부**에 적용된다(2026-09-05 에 두 번 넓혔다 — ADR-0171 의
   *  두 amendment). 단 **미도달은 판정 시점이 캔들 선과 다르다**: 캔들 선은 「지금도
   *  미도달인 벽」(하루 스코프·소급 재분류), 이 pane 은 「그 봉 시점에 미도달이던 벽」
   *  이다. 후자가 아니면 과거 봉의 값이 장중에 사라진다.
   *
