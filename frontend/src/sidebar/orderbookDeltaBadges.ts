 ...
 * 잠깐 표시했다가 TTL 만료 시 걷어낸다. 값의 의미는 "직전 수신 스냅샷 대비" — WS push 가
 * 절사되므로 놓친 중간 전환은 담기지 않는다(그건 차트 지표의 버킷 합이 담당).
 */
export type OrderbookDeltaBadge = {
  delta: number;
  /** 뱃지 생성 벽시계(ms) — TTL 만료 판정 기준. 스냅샷 t_ms 는 초 해상도라
...
/** 뱃지 표시 수명 — 이 시간이 지난 뱃지는 다음 갱신에서 정리한다. 페이드아웃을
 *  제거한(2026-07-21) 뒤로는 이 상수가 **표시 시간의 유일한 통제점**이다
 *  (예전엔 CSS 페이드 1.5s 와 동기화된 값이었다). */
 ...
 *  (표시 시간 갱신), TTL 을 넘긴 뱃지는 정리한다. 변화가 없으면 `state` 그대로 반환
 *  (참조 안정 → 하류 memo 유지). */
