...
it('autoOpen 이면 클릭 없이 팝오버가 열리고, 닫으면 다시 열리지 않는다', () => {
  const { rerender } = render(<FolderAddButton folderId="f1" autoOpen />);
  ...
  // 마운트 시 초기값으로만 읽는다.
  rerender(<FolderAddButton folderId="f1" autoOpen />);
  expect(screen.queryByRole('dialog', { name: '종목 추가' })).toBeNull();
});
...
it('자동으로 열렸으면 onAutoOpened 로 1회 통지한다', () => {
  ...
  const { rerender } = render(<FolderAddButton folderId="f1" autoOpen onAutoOpened={onAutoOpened} />);
  expect(onAutoOpened).toHaveBeenCalledTimes(1);
  // 페이지가 표식을 태운 뒤(autoOpen=false) 다시 부르지 않는다.
  rerender(<FolderAddButton folderId="f1" onAutoOpened={onAutoOpened} />);
  expect(onAutoOpened).toHaveBeenCalledTimes(1);
});

it('autoOpen 없이 마운트되면 닫힌 채로 시작하고 통지도 없다', () => {
  const onAutoOpened = vi.fn();
  render(<FolderAddButton folderId="f1" onAutoOpened={onAutoOpened} />);
  expect(screen.queryByRole('dialog', { name: '종목 추가' })).toBeNull();
