...
//
// **단언이 하나도 없는 벤치마크다** — 숫자를 console 로 찍을 뿐 아무것도 주장하지 않는다.
// vitest 요약에 "8 passed" 로 잡히지만 그건 커버리지가 아니다. 벽시계 비율을 단언으로
// 세우면 공유 러너에서 flake 가 되기 때문에 의도적으로 이렇게 둔다
// (candle.perf.test.ts 가 그 이유로 2026-07-31 에 호출 횟수 계약으로 바뀌었다).
//
...
describe('라이브 호가 보조지표 — 틱당 처리 비용 (캔들/거래량 제외)', () => {
  ...
  it('G) 총잔량 급증 마커 — 틱당 비용 (Split Cache seam vs 전구간 재계산, days 스윕)', async () => {
    ...
    const ctx = { auctionMask: true, surgeEnabled: true, surgeApproachPct: 95, surgeRearmPct: 85, surgeStartHHMM: 900, tickNormalize: false, surgeTickConfirmPct: 10 };
    for (const days of [1, 5, 30, 90]) {
      const { bundle, axis, pointCount } = makeBundle(days);
      ...
      const tFull = median(() => {
        detectSurgeSide(bundle.quote_ratio.points, 'ask', { approachRatio: 0.95, rearmRatio: 0.85, isClosingAuction: (t) => axis.inClosingAuctionWindow(t) });
        detectSurgeSide(bundle.quote_ratio.points, 'bid', { approachRatio: 0.95, rearmRatio: 0.85, isClosingAuction: (t) => axis.inClosingAuctionWindow(t) });
      });
      // eslint-disable-next-line no-console
      console.log(
