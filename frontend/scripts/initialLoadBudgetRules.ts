...
export const INITIAL_LOAD_BUDGET = {
  ...
  //
  // raw 1,311 → 1,313 KB (2026-08-22): 창별 hogaplay 소스의 **키움 보충 합류**
  // (게이트 확장 · 누적 정책 재작성 · 빈 상태 축 하나 · 칩 툴팁 요약). 실측 내 몫
  // **raw +1.4 KB / gzip +0.4 KB**(내 base 3bd17070 단독 1,309.8 / 394.1 →
  // 1,311.2 / 394.5). 종전 헤드룸이 1.2 KB 라 넘었다.
  //
  // **보충 훅 자체는 이미 초기 그래프 안에 있었다**(`useLiveBundle` 이 저장뷰용으로
  // 부르고 있었다) — 그래서 늘어난 것은 기능 한 벌이 아니라 **배선과 문구**다.
  // 큰 쪽이 문구인 것도 앞선 #1493 항목과 같다: 빈 상태 한 줄과 보충 요약 네 갈래
  // (보충함 · 보유 밖 · 척도 불일치 · 유예)가 전부 한국어 문자열이라 UTF-8 3바이트를 문다.
  //
  // 줄일 여지를 먼저 봤고 둘 다 막혔다: ① **지연 로드 불가** — 보충은 훅이라 조건부
  // import 가 훅 규칙 위반이고, 빈 상태·칩은 차트 창이 뜨는 순간 필요하다.
  // ② **문구를 깎는 것이 곧 정책을 깎는 것**이다 — 사유 네 갈래를 한 문장으로 뭉치면
  // 사용자가 할 수 있는 일(재시도할 것 · 영영 없는 것 · 더 좁게 보면 되는 것)이
  // 구별되지 않는다. 그게 이 PR 의 존재 이유이기도 하다(종전엔 채우지도 말하지도
  // 않았다). 새 헤드룸 1.8 KB — 이 파일의 앞선 항목들(0.4~1.9)과 같은 자릿수다.
  rawBytes: 1_313 * 1024,
  /**
