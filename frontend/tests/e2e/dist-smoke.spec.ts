 ...
 *
 * 이 스펙만 vite(5174)가 아니라 **백엔드 8765 를 직접** 본다 — 백엔드가
 * HOGA_FRONTEND_DIST=/tmp/hoga-e2e-dist 로 빌드 산출물을 same-origin 서빙하는
 ...
 */
const DIST = 'http://127.0.0.1:8765';
...
test.describe('prod dist serving (ADR-0134)', () => {
  test('smoke: SPA 딥링크 → config.json → 첫 렌더 → same-origin WS 생존', async ({
    ...
    // WS 가 same-origin 절대화(ws://…:8765/api/ws)로 열리고 **살아남는지**.
    ...
    expect(ws.url()).toBe('ws://127.0.0.1:8765/api/ws');
