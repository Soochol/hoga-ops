...
**기간내 거래대금 → 날짜 범위**에서도 시작일·종료일과 최소 거래대금(억)을 지정할 수 있다.
양 끝 날짜를 포함해 하루라도 기준을 충족하면 검색되며, 기간 합계가 아닌
일별 추정 거래대금 `(시가+고가+저가+종가)/4 × 거래량`으로 판정한다.
이력이 부족하면 같은 **과거 일봉 수집** 기능을 사용하며, 비교 기간 없이 지정한
기간의 일봉만 필요하다. 결과와 CSV에는 발생 건마다 해당 충족일과 추정 거래대금을 표시한다.
기존 **최근 N일** 방식과 저장한 조건도 그대로 사용할 수 있다.

검색 결과는 **발생 건마다 한 행**으로 표시하며 각 행에서 조건·발생일별로 제외할 수 있다. 같은 종목의 다른 날짜나
다른 조건은 유지되며, **제외한 발생 건**에서 개별 복원할 수 있다. 제외 기록은 서버에 저장된다.
발생일·당시 수치를 현재 시세와 구분하고, 선택·CSV도 발생 건 단위로 처리한다.
