...
- **승격 주기를 너무 길게 설정하면 당일 차트 이음새에 빈 구간이 생길 수 있던
  잠재 문제**: 당일 데이터 승격 주기(`HOGA_LIVE_TODAY_PROMOTE_INTERVAL_S`)를
  10분 이상으로 올리면 메모리 버퍼(15분 보존)가 못 메우는 데이터 hole이 조용히
  생길 수 있었다. 이제 안전 상한(600초)을 넘는 설정은 경고와 함께 기본값(300초)
  으로 자동 보정된다.
- **/live 상태바에 'LIVE●'가 두 번 나란히 표시되던 글리치 해소**: 연결 상태
  표시와 캡처 헬스 표시가 모두 'LIVE●'를 써서 중복처럼 보이던 문제. 캡처 쪽을
  '수신●'으로 구분해 두 신호(연결 vs 수신)가 명확히 분리된다.
