...
### Fixed
- **매 거래일 개장 직후 ~30초 실시간 데이터 유실 수정**: KIS WebSocket 연결
  게이트가 장 시작 전 30초 간격으로만 깨어나, 09:00 개장 시 연결·구독이 최대
  30초 늦어 하루 중 체결이 가장 활발한 구간을 매일 놓치던 문제. 이제 게이트가
  1초 간격으로 깨어나 개장 ~1초 내 연결된다.
- **거래원(회원사) 이름이 실시간과 과거 화면에서 다르게 표시되던 문제**: 실시간
  거래원명이 KIS 원본 표기('신한증권')로, 과거 차트는 정규명('신한투자증권')으로
  나와 같은 거래원이 둘로 갈리던 문제. 실시간 파싱 시점에 정규화해 실시간·과거가
  같은 이름으로 통일된다.
- **관심종목 편집 중 연결 끊김이 겹칠 때 캡처 종목이 어긋나던 문제**: 관심종목
  순서를 바꾸는 순간 WebSocket 송신이 실패하면, 갱신된 종목 집합이 일부만
  반영돼 옛 종목이 계속 수집·승격되던 문제. 핵심 상태를 먼저 확정하도록 순서를
  바꿔, 송신이 실패해도 수집 대상이 정확히 갱신된다(연결은 재연결로 자가 복구).
