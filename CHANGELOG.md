...
### Changed
- **실시간(/live)에서 거래원 궤적이 당일 전체로 표시**: 그동안 /live를 실시간으로
  보는 동안 거래원(회원사) 매집 궤적이 최근 15분만 그려졌다(메모리 버퍼 한계).
  이제 당일 09:00부터의 전체 궤적이 표시된다 — 누가 장 시작부터 매집해왔는지
  실시간으로 볼 수 있다. 서버가 승격된 당일 데이터(parquet)와 라이브 버퍼의
  미승격 꼬리를 이어 붙이고(약 60초 주기 갱신), 거래원 카드가 차트와 같은 데이터
  소스를 사용하도록 정합. 과거 차트(스크럽)는 이전과 동일하게 당일 전체를 표시.

> 참고: 실시간 시각 검증(09:00부터 그려지는지·꼬리 전진·이음새 연속)은 다음 평일
> 장중(09:00–15:30)에 수행 예정 — 백엔드·프론트 로직은 유닛 테스트로 검증 완료.
