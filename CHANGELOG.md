...
### Changed
- 프로그램 장중 수급을 시장별 최대 12페이지까지 조회하고, 제공 시간 범위와 조회 한도 도달 여부를 표시한다.
- 투자자·프로그램 일별 수급에 5·20·60일 선택과 실제 제공 기간을 표시한다.
- 수급 차트에 시점별 값 탐색과 수신 실패 구간 안내를 추가하고, 좁은 화면에서도 눈금과 툴팁이 겹치지 않도록 개선한다.
- 업종 목록에 업종·대표 지수·미분류 필터와 전체 보기를 추가한다.
- 시장 카드에 데이터 기준 시각과 API 연결 실패 안내를 보강한다.
...
### Changed
- 시장 투자자·프로그램 일별 수급을 날짜별 숫자 목록으로 표시하고, 열 제목과 기간 합계를 스크롤 중에도 유지한다. 배경색 강조를 줄이고 숫자를 오른쪽으로 정렬한다.
- 거래대금 추이를 공통 눈금의 일별 막대로 표시하고, 최신 거래일 거래대금과 이전 거래일 평균을 구분한다.

### Fixed
- 시장 당일·일별 전환 시 카드 높이 변화로 화면이 흔들리는 문제를 해결한다.
- 공급사 날짜 형식과 누락값을 올바르게 표시하고, 당일 잠정 데이터와 확정 데이터를 구분한다.
...
### Added
- 스크리너 결과를 조건·종목·발생일별로 펼쳐 특정 발생 건만 제외하고, 제외 목록에서 개별 복원하거나 되돌릴 수 있다. 같은 종목의 다른 날짜와 다른 조건은 유지한다.
- 제외 기록은 재조회·새로고침과 페이지·우측 패널에서 공유된다. 제외 후 결과를 갱신해도 결과 내 검색어·선택·펼침 상태를 유지한다.

### Changed
- 조건별 전체 발생일을 보존하고 제외를 적용한 뒤 조건 충족 여부와 결과 상한을 계산한다. CSV에도 남아 있는 발생 건을 기록한다.
...
### Fixed
- 과거 신고거래량 검색에서 달력이 일부 부족해도 평가 가능한 후속 구간의 일봉을 수집하고 재사용한다.
- 한 종목의 데이터 불일치나 재조회 실패가 같은 배치의 정상 종목 저장을 막지 않도록 한다.

### Changed
- 조건별 날짜 비교 창을 재사용하고 배치 병합의 전체 데이터 반복 처리를 줄여 과거 검색·수집 비용을 낮춘다.
- 수집 작업 상태와 검증·저장 책임, 공통 조건 비교 규칙을 분리하며 기존 작업 기록과 응답 형식을 유지한다.
...
### Fixed
- 좁은 10호가 창에서 숨겨진 행·열을 안내하고 매도·매수 및 잔량·요약 이동 버튼으로 전체 데이터를 확인할 수 있다.
- 밝은 테마에서 보이지 않던 120 이동평균선과 신규 지표 슬롯의 흰색 기본값을 회색으로 바꾼다. 저장된 색은 유지하고 대비가 낮으면 색상 팔레트에서 회색 적용을 제안한다.

### Changed
- 히트맵 헤더의 수집 지연 라벨을 숨기고 보충 수집은 그룹 메뉴에서 제공한다.
- 히트맵 그룹명·평균 기준·수집 지연을 명확히 표시하고 지연 종목 보충 수집, 그룹 선택 검색, 일치 종목만 보기와 키보드 그룹 메뉴를 제공한다. 좁은 화면에서 툴바와 보드를 재배치한다.
- 시장 종합 카드에 기준일·조회 시각과 표 열의 의미를 표시하고, 자금 잔고와 기간 증감을 구분한다. 차트는 마우스·방향키로 상세 값을 확인할 수 있다.
- 시장 종합 거래대금 순위에 금액을 표시하고, 업종 정렬·일별 수급 바로가기·좁은 화면 재배치와 버튼 클릭 영역을 개선한다. 프로그램 시간축은 시간외 표본까지 포함한다.
- 종목 검색에 적용할 링크 그룹과 고정 창을 제외한 연결 창 수를 표시하고, 빈 데이터 창에서 종목 선택 방법을 안내한다.
- 창 닫기·그룹·고정 버튼의 클릭 영역을 넓히고 닫기와 그룹 버튼의 접근성 이름을 명확히 한다.
- 관심종목의 수집 카운트다운을 '다음 수집까지'로 표시해 예약 시각과 구분한다.
...
### Added
- 저장뷰 우측 패널에서 같은 종목의 여러 저장뷰를 선택해 원하는 위치로 옮기고, 목적지·기간 안내와 순서 되돌리기를 사용할 수 있다.
- 선택한 저장뷰를 Delete 키나 선택 삭제로 지우고, 패널을 닫은 뒤에도 요청별로 5초 동안 실행 취소할 수 있다. 일부 삭제 실패는 실패 항목만 재시도할 수 있다.

### Changed
- 종목 그룹·저장뷰 이동을 전용 핸들로 분리하고 가장자리 자동 스크롤을 지원한다. 검색·이름 정렬 중에는 순서 변경 제한을 안내한다.

### Fixed
- 검색 시 접힌 일치 그룹을 임시로 펼치며, 검색을 지우면 기존 접힘 상태로 복원한다.
- 초기 조회 중 기존 순서·접힘 설정을 보존하고, 저장 실패와 삭제 후 편집 상태·키보드 포커스를 처리한다.

## [0.13.1.0] - 2026-09-08

### Added
- 히트맵 우측 패널에서 여러 종목을 선택해 원하는 위치로 이동·Ctrl 복제하고, 변경을 되돌릴 수 있다.
- 이동·복제 목적지와 중복 처리, 저장 결과·실패를 안내한다. 접힌 그룹 임시 펼침과 가장자리 자동 스크롤을 지원한다.

### Changed
- 종목·그룹 드래그를 전용 핸들로 분리하고, 정렬 중 그룹 이동과 현재 그룹 내 Delete를 유지한다.

### Fixed
- 그룹 간 삽입과 되돌리기를 원자적으로 저장하고, 후속 편집과 충돌하면 덮어쓰지 않는다.
- 패널 밖 드롭을 취소하고, 스크롤 후 고정된 그룹 헤더의 실제 위치를 기준으로 드롭한다.

## [0.13.0.0] - 2026-09-08

### Added
- 관심종목 패널에서 여러 종목을 선택해 그룹 사이의 원하는 위치로 한 번에 이동하고, 중복 소속을 합친 뒤 되돌릴 수 있다.
- 드래그 목적지와 저장 결과·실패를 안내하며, 접힌 그룹 자동 펼침과 가장자리 자동 스크롤을 지원한다.

### Changed
- 종목·그룹 이동을 전용 핸들로 분리하고, 등락률 정렬 중에도 그룹 간 이동을 허용한다. 그룹 헤더에 놓으면 끝에 추가한다.

### Fixed
- 그룹 이동을 한 번에 저장하고, 되돌리기가 이후 편집을 덮어쓰지 않도록 검증한다.
- Delete 키는 포커스된 행의 현재 그룹 소속만 해제하고 다른 그룹의 같은 종목을 유지한다.

## [0.12.48.1] - 2026-09-08

### Changed
- 스크리너 페이지와 우측 패널의 조건검색 선택기를 검색·키보드 조작이 가능한 공통 목록으로 통일했다. 긴 이름과 현재 선택을 확인할 수 있고, 새 조건 만들기를 별도 버튼으로 분리했다.

### Fixed
- 브라우저 기본 선택 팝업을 제거하고, 검색창·목록 클릭과 화면 갱신 중에도 목록 열림을 유지하도록 했다.
- 조건 전환·새 조건·원본 복원에서 미저장 변경을 보호하고, 저장 중 조건 전환을 막았다. 명시적으로 만든 빈 초안은 재진입 후에도 유지한다.

## [0.12.48.0] - 2026-09-08

### Added
- 스크리너 결과에서 현재 시세와 조회 당시 가격·등락률을 전환하고, 조회 가격의 기준일을 확인할 수 있다.
- 결과 내 종목명·코드 검색과 다중 선택, 검색 결과 전체 선택, 선택·검색 결과 CSV 내보내기를 추가했다. CSV에는 조회 당시 값과 현재 시세, 실행 조건과 조회 시각을 함께 보존한다.

### Fixed
- 가상 목록의 스크롤 기준을 수정해 긴 결과 목록의 마지막 행이 누락되던 문제를 해결했다.
- 조회 중 조건을 편집해도 완료된 결과에는 실행 당시 조건과 저장본 정보를 보존한다.
...
### Changed
- 라이브 분봉 첫 조회에서 오늘 캔들을 먼저 표시하고 과거 봉을 이어서 채운다. 오늘 전용 조회의 수정계수 요청을 생략하고, 오늘 조회 실패·빈 응답에서도 과거 로딩을 계속한다.
- 최대벽·히트맵·거래원·프로그램 지표를 캔들과 동시에 요청해 먼저 준비한다. 가격 범위가 필요한 매물대·POC는 별도로 요청하며, 과거 병합 시 화면 위치와 수정주가 환산을 유지한다.
- 과거 1분 최대벽 캐시 사전 준비에 히트맵을 포함해 다음 조회와 큰 분봉의 원본 재계산을 줄인다.
...
- 최대벽 봉별 모드에서 사용하지 않는 계단 계산을 생략하고, 계열을 켜고 끈 변경을 즉시 반영한다.

## [0.12.34.0] - 2026-09-05

### Changed
- 패턴 검색 조건을 빠르게 바꾸면 마지막 입력만 요청하고, 닫은 패널의 대기 검색을 취소한다. 동일 조건의 동시 검색은 계산을 공유한다.
- 과거 패턴 검색에서 선택 기간 밖의 봉은 계산 전에 제외한다. 합성 100만 봉의 최근 252봉·이평 5/20·길이 ±2 검색은 코퍼스 캐시가 준비된 상태에서 484→72ms로 줄었으며 결과 동등성을 검증했다.
- 같은 종목을 여러 실시간 창에서 볼 때 버퍼와 갱신 타이머를 공유한다. 초기 데이터와 실시간 틱이 같은 시각에 겹쳐도 서로 다른 체결·호가와 반복 횟수를 보존하고, 자정에는 새 거래일로 전환한다.
