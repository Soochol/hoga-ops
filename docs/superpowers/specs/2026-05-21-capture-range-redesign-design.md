...
- **Rename `is_partial_capture` to disambiguate from data-`is_partial`** (carried over from `/improve-codebase-architecture` grill #4). The orchestrator's `is_partial_capture(date, now) → bool` (policy check: today + before close) and the meta-field `is_partial` (data quality: gaps in Data Window) share a word but are orthogonal concepts. Since this Q14 rewrites the policy path anyway (`allow_partial` removed, 18:00 backend guard added), drop `is_partial_capture` entirely and replace with a function named after the new semantic: `is_today_too_early(date, now) → bool` returning True when `date == today_kst AND now.hour < 18`. The data-side `is_partial` keeps its name (it accurately describes a Data Window with gaps). `PartialCaptureRefused` becomes `TodayTooEarlyRefused`. After Q14 lands, "partial" appears in exactly one meaning across the codebase: the data-completeness bit.
