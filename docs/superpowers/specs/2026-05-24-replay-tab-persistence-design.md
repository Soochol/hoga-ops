...
3. **버전 키**: 스토리지 키 자체에 버전 포함(`replay.session.v1`). 스키마 깨지면 키 바꿔서 자연스럽게 폐기.

### Storage schema

**Key:** `replay.session.v1`
...
type ReplaySessionSnapshot = {
  ...
  activeIndex: number;        // tabs 내 인덱스 (id가 아닌 인덱스)
  tabs: PersistedTab[];       // 빈 배열 가능
};
```
...
**유효성 기준:** JSON 파싱 성공 + `version === 1` + `tabs`가 배열 + 각 entry의 selection이 `null`이거나 4개 필드 모두 valid. 한 entry라도 invalid면 그 entry만 drop. 전체가 비면 fallback.
...
| 저장된 timeframe이 유효하지 않음(스키마 진화) | 해당 entry drop. 모두 invalid면 빈 탭 1개로 fallback |
| 빈 탭 목록 저장 (사용자가 모든 탭 닫음 — 현재는 불가능, `closeTab`이 `length <= 1`이면 no-op) | 발생 불가, 단 방어적으로 빈 배열도 허용하고 fallback으로 빈 탭 1개 시드 |
...
9. URL 있음 + localStorage 있음 → URL이 이김
10. URL 없음 + localStorage 있음 → localStorage 복원
11. URL 없음 + localStorage 없음 → 빈 탭 1개
12. 탭 추가 → 250ms 뒤 localStorage 반영
13. 탭 닫기 → 250ms 뒤 localStorage에서도 제거
14. prefs 변경(MA period) → 250ms 뒤 반영

### 수동 검증
15. 브라우저 종료 후 `/replay` 깨끗 진입 → 마지막 세션 복원
16. 좌측 nav `/inventory` → `/replay` → 마지막 세션 복원
17. 시크릿 창 → 빈 탭 1개
18. DevTools에서 localStorage 키 수동 손상 → 새로고침 시 깨끗하게 복원, console에 warn
