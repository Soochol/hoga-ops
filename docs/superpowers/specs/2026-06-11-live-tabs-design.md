...
> **개정 안내(2026-06-12):** 본 스펙의 §6 클릭 내비게이션(`openOrFocusTab` = open-or-focus, 종목당 탭)은
> 이후 **단일-탭 모델**(`setActiveTabCode` = 현재 탭 교체, 새 탭은 `+`만, + 관심종목 드래그→차트 드롭)로
> 개정됐다. 최신 동작은 **ADR-0069 Addendum(2026-06-12)** 참조 — 아래 §6/§14의 open-or-focus 서술은 구식이다.
