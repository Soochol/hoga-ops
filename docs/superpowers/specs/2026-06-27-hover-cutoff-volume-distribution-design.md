...
**Date**: 2026-06-27
**Status**: Approved
**Canonical term**: 연속체결 매물대 분포 hover cutoff
...
- Apply the same behavior to `/live` and `/study`.
...
- It is valid in `full` and `sidecar` modes, but frontend uses it through a sidecar request.
...
The helper converts `cutoff_ms` to the same intra-day time scale used by `query_continuous_trade_volume_distribution`. The effective upper bound becomes:
...
For `/study`, `StudyReferenceDetailPanel` uses the same cutoff sidecar query keyed by the saved view's code/timeframe/source inputs and the active cursor.
...
- Keep placeholder/fallback final profile visible while the cutoff query is loading.

If testing shows excessive request churn, add a small debounce or keep previous cutoff data while fetching. Do not precompute all cutoff distributions in the main bundle; that would make normal range loads too heavy.
...
- `/study` with toggle on and cursor active prefers the cutoff sidecar profile.
