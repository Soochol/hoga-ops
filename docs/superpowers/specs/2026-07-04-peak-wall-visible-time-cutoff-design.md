...
**Date**: 2026-07-04
**Status**: Draft
**Scope**: `frontend/src/live/indicators/AskPeakConfig.tsx`, `frontend/src/live/indicators/BidPeakConfig.tsx`, `frontend/src/state/chartPrefs.ts`, `frontend/src/live/LiveChartRoot.tsx`, `frontend/src/live/useDayAskPeaks.ts`, `frontend/src/live/useDayBidPeaks.ts`
...
- `askPeakVisibleTimeCutoff`, label `보이는 최신 시각 기준`, default `false`.
- `bidPeakVisibleTimeCutoff`, label `보이는 최신 시각 기준`, default `false`.
...
- If cutoff is enabled but no visible candle time can be derived, fall back to the current behavior for loaded data rather than throwing.
- If candidate arrays do not include enough timestamped rank data for a historical day, keep the best timestamped candidate that is available and add tests around known supported shapes. Do not invent a synthetic timestamp.
...
- On `/live`, select a stock with minute data and enable only `당일 매도 최대벽` → `보이는 최신 시각 기준`.
