# Inventory: per-Stock-Date 캡처 횟수 컬럼
...
`/inventory` 우측 상세 테이블은 한 종목의 (code, date) 행들을 나열하지만,
"이 Stock-Date가 평생 몇 번 캡처됐는지"를 알 수 없다. 사용자는 어떤 날짜를
얼마나 자주 재캡처하고 있는지 한눈에 보고 싶어한다.

ADR-0031의 `QueueItem.attempt`는 **한 캡처 작업 안에서의 재시도 횟수**를 의미한다.
큐가 drain되면 사라지므로 inventory의 영구 컬럼이 될 수 없다. 우리는 새로운
디스크 카운터가 필요하다.

## 2. 도메인 용어

- **capture_count** (신규): Stock-Date 디렉토리에 meta.json이 성공적으로
  쓰여진 누적 횟수. 초기 캡처 = 1, 재캡처 성공 = 직전값 + 1.
- **attempt** (기존, ADR-0031): 한 캡처 작업 안에서 Retry 클릭으로 누적되는
  재시도 횟수. 큐가 drain되면 사라진다.

두 카운터는 **직교**한다:
- 초기 캡처가 3번 재시도 끝에 성공 → `attempt=3`, `capture_count=1`
- 다음 달 재캡처가 한 방에 성공 → `attempt=1`, `capture_count=2`

UI는 두 값 모두 `×N` 뱃지로 표시하되 위치가 다르다 — `attempt`는
[CaptureQueueRow.tsx:53](frontend/src/capture/CaptureQueueRow.tsx#L53) 큐 행에,
`capture_count`는 inventory 상세 테이블에 표시.
...
- `capture_count: int` — 옵셔널 필드. 1 이상의 정수.
...
1. `prior = json.loads((out_dir / "meta.json").read_text(...)).get("capture_count")`
   - 파일 없거나 필드 없으면 `None`.
2. `meta["capture_count"] = (prior or 0) + 1`
...
return StockDate(
    ...,
    capture_count=meta.get("capture_count"),
)
```
...
`frontend/src/inventory/sortDates.ts`에 `captureCount` SortKey 추가:

- 첫 클릭: `desc` (높은 횟수 우선 — 자주 재캡처한 날짜 위로).
- `null`은 항상 마지막 (방향 무관).
...
- parser: 새 디렉토리 첫 캡처 → meta.json에 `capture_count == 1`
- parser: 기존 meta.json `capture_count: 2` → 재캡처 후 `3`
- parser: 기존 meta.json에 카운터 없음(legacy) → 재캡처 후 `1`
- queries: meta에 필드 없으면 `StockDate.capture_count is None`
- queries: meta에 `capture_count: 5` → `StockDate.capture_count == 5`
...
- 기존 픽스처에 `capture_count: null` 명시적 추가 (옵셔널이지만 명시)

## 6. 마이그레이션 & 운영

- **백필 없음.** 새 캡처가 일어나야 카운터가 자라기 시작.
- 기존 Stock-Date들은 `—`로 표시되다가, 재캡처될 때 카운터가 ON된다.
- 롤백 안전: 추가 필드는 additive — 코드 롤백해도 디스크 잔류 무해.
- FE 롤백 시 컬럼만 사라짐.

## 7. 비목표 (Non-goals)

- 캡처 타임라인/이력 (언제 each capture 일어났는지) — 후속 작업.
- 좌측 종목 카드 집계 — 별도 요청 시 추가.
- `attempt`와 `capture_count`의 통합 — 의미 충돌로 의도적 분리.
- 백필 마이그레이션 — YAGNI.
...
- ADR-0020 (DiskState)
- ADR-0031 (`attempt` 의미론 — capture_count는 이와 별개)
