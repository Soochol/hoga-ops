...
- **장 외**: 차트는 마지막 가용 Live Session 데이터 표시 + "장 외 시간입니다 (09:00 시작)" 배너 _(Update 2026-05-31: 장 외 배너는 이후 제거됨 — 폴링이 서버 시계(`poller.py:_should_poll_now`)로 게이팅되어 운영상 불필요. 마지막 데이터 표시는 유지, 배너만 삭제.)_
