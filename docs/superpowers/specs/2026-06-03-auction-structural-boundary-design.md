...
**검증 (전 종목 교차):** 3호가 시그니처(양쪽 4~10호가 잔량 모두 0)는 유동 종목
**장중(09:05–15:15) 거짓양성 0건**. 장전 동시호가·장 시작 직후 과도구간(~5초)·
장중 VI 단일가·장마감 동시호가에서만 출현. 즉 구조 신호는 *모든* 단일가 구간을
잡는다 — 본 spec은 그중 **장마감 동시호가만** 선별한다(Goals/Non-Goals 참조).
...
- **반장(Half-Day) 자동:** 마감 오프셋(`−10분`)을 쓰지 않으므로 12:30 마감일도
  자동 대응. 임계값이 ~12:20 직전 마지막 연속 스냅샷으로 따라간다.

`last_continuous_ms` 산출은 **Regular Session 상한(`≤ session_close_ms`)으로 한정**한다 —
After-Hours Trading(15:30–16:00 시간외)의 호가 스냅샷이 임계값을 장마감 동시호가
*뒤로* 밀어버리는 것을 방지(그러면 15:20–15:30이 임계값 이전이 되어 제외 실패).
...
| Half-Day 안전성 | **preserves + 강화** | 백엔드·**프론트 live 모두** 마감 오프셋 불필요 → 직전 spec의 프론트 half-day known-limitation 자연 해소(Risks 참조). |
...
- **반장 자동 해소:** 직전 spec의 known-limitation(프론트가 오늘 `close_ms`를
  15:30으로 폴백 → 반장 미정화)이 사라진다. 구조 임계값은 마감 시각의 *정확한*
  값에 의존하지 않고(상한으로만 사용), 실제 호가창 전환을 직접 읽기 때문. 반장
  오늘분도 ~12:20 마지막 연속 스냅샷으로 정확히 임계값이 잡힌다. (`close_ms`가
  15:30 폴백이어도 상한이 느슨해질 뿐 — 12:20 전환은 그 안에서 정상 검출.)
...
- **VI 단일가 데이터 유지의 trade-off (의도):** 사용자 선택 "장마감만"에 따라 장중
  VI(예: 11:39–11:49 3호가)는 계산에 *유지*된다. VI 구간 호가비·총잔량은 3호가
  부분-호가창 합이라 아티팩트(편향 스파이크)가 남을 수 있다. 후속으로 "모든 단일가
  제외"를 원하면 `is_continuous`/`isContinuousBook` 판정을 임계값 없이 per-row로
  적용(`intra_ms > last_continuous_ms` 제약 제거)하는 1줄 전환으로 처리 가능.
...
- **표시 경계 구조 재앵커** — `RangeSegment.auction_start_ms`(데이터 파생 1필드)를
  ...
  경계분 완전 정합. (v1은 계산만.)
- **모든 단일가 제외 옵션** — VI·장전까지 빼는 순수 구조 검출 토글(위 trade-off).
