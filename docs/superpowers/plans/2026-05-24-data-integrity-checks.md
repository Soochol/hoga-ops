...
def _collection_unique_events_ratio(m: dict) -> Violation | None:
    # Stagnation = many pages with few unique events. Tiny captures (test
    # fixtures, halts, very short trading days) carry no signal — skip below
    # the page floor so they don't false-positive.
...
def classify_from_meta(meta: dict[str, object]) -> DiskState:
    ...
    Priority (ADR-0020):
      1. Any ``error``-severity invariant violation → INVALID. Broken data
         ...
         everything: forces eligibility to fresh-capture instead of resuming
         a corrupted parquet, and lets ``build_range_bundle`` skip the
         segment regardless of whether collection completed.
      2. ``collection_complete=False`` → CLIENT_INCOMPLETE.
