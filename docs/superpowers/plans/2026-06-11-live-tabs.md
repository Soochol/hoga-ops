...
> **개정 안내(2026-06-12):** 본 플랜이 배선한 `openOrFocusTab`(open-or-focus)은 이후 `setActiveTabCode`
> (현재 탭 교체) + `addBlankTab`(+ 버튼) + 관심종목 드래그→차트 드롭으로 개정됐다. 최신 동작·근거는
> **ADR-0069 Addendum(2026-06-12)** 참조.
...
- **드래그 시각 affordance** (Task 8 리뷰 발견) — 현재 네이티브 드래그는 기능만 동작하고 drop-indicator(어디 떨어질지)·`cursor: grab`·드래그 중 opacity가 없다. DESIGN.md가 드래그 비주얼을 명시 안 해 v1 허용. 후속으로 drop-indicator 추가 권장.
- **`liveTabs.ts` 모듈-로드 부작용 정리** (Task 4 리뷰 발견) — import 시 `attachPersistence`+`useLivePageStore.subscribe`가 실행돼 무관 테스트에 파급(Task 2·Task 4). (a) **✅ 완료 (C2 deepening, 커밋 `8fed278`)** — subscribe/persistence 와이어링을 `initLiveTabsSync()`(멱등)로 빼 main.tsx에서 1회 호출. import-시 부작용 제거(loadTabs 읽기만 잔류). (b) **남은 follow-up** — `openOrFocusTab`/`applyTabToPage`가 매 클릭 `useLivePageStore.getState()`를 호출하므로 selector-only mock 소비자 테스트는 여전히 `liveTabs` mock 필요; 이를 `useJumpToLive` mock으로 표준화하면 파급 근본 종결. 단일 SPA라 긴급도 낮음.
