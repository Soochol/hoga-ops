...
it('renders one element per tab with code and name', () => {
  setup();
      ...
      {tabs.map((t) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-sm font-mono tabular-nums shrink-0" style={{ color: active ? 'var(--fg-dim)' : 'var(--fg-dimmer)' }}>{t.code}</span>
            {t.label !== t.code && (
              <span className="text-sm shrink-0" style={{ color: active ? 'var(--fg)' : 'var(--fg-dim)' }}>{t.label}</span>
            )}
            <button
              type="button"
