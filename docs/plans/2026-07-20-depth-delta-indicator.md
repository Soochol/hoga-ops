...
> ⚠️ **폐기됨 (2026-08-25) — 이 지표는 제거됐다. ADR-0161 참조.**
> 아래 본문은 그때의 설계 그대로 보존한다. 다시 만들 일이 생기면 §2(델타의 정의)와
> §5(과거일 소스가 없는 이유)를 먼저 읽을 것 — 같은 함정을 다시 밟는 것보다 싸다.
> 살아남은 것은 **10호가 카드의 증감 뱃지**(`sidebar/orderbookDeltaBadges`)뿐이고,
> 그건 이 문서의 차트 오버레이와 **다른 표면**이다.

날짜: 2026-07-20
상태: **폐기** (2026-08-25 제거). 한때 구현 완료였다 (v1 — 오늘/분봉 전용).
