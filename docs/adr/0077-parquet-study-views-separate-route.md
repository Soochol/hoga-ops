...
Detail enrichment is best-effort at save/update time. Missing parquet files, missing continuous orderbook representatives, or broker detail gaps do not reject the save; they are recorded in `bundle.detail_warnings` and represented by unavailable dense detail buckets. Loading a saved view must not use parquet as a repair path: legacy snapshots without detail arrays reopen as their persisted JSON artifact with unavailable detail.
