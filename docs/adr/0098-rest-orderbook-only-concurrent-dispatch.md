...
**Amended (2026-07-10 — ADR-0100):** Context 근본원인 2("콜레이트 상한 15콜/s 명의-전역")와 "정정(CONTEXT.md 갱신)" 단락은 폐기됐다 — 재실측으로 상한은 **앱키별 ~15/s**라 3계정이면 예산 ~45콜/s(30초당 ~1350콜). 결정 1·2·3(호가 전용, 동시 디스패치, 벽시계 경계)은 그대로 유효하며 예산 산식만 유리해진다. 비범위의 "다중 명의로 콜레이트 증설"도 같은 명의 계정 추가로 대체 가능해졌다.
