...
### 갱신 (2026-08-20, 행 ctrl/⌘+클릭 = 브라우저 새 탭)

**§5 는 그대로다.** 저장뷰 드로어의 행 ctrl/⌘+클릭이 돌아왔지만, 되돌아온 것은 §5 가
제거한 그것이 **아니다** — 이 구별이 이 갱신의 전부다.

- **§5 가 제거한 것**: *앱 안* 탭으로 여는 disposition. `openSaveInNewTab`, 탭 배열,
  탭 스트립. 그 판정은 유지된다 — 이번 변경은 탭 스토어도, 멀티 뷰 상태도, disposition
  인자도 하나 되살리지 않는다. 열리는 `/study` 는 여전히 저장뷰를 **한 번에 하나** 본다.
- **이번에 추가한 것**: **브라우저** 탭. `window.open(studyViewDeepLinkPath(id), '_blank',
  'noopener')` 뿐이고, 그 새 탭이 타는 경로는 §7 이 저장뷰 전환의 정식 경로 둘 중 하나로
  못박은 **`?view=` 딥링크**다. `/live` 가 같은 제스처에 대해 쓰는 `openLiveInNewTab` 의
  `/study` 짝이다(`studyViews/studyDeepLink.ts`).

**추가되는 영속 상태는 0이다.** 새 탭은 자기 sessionStorage 를 갖고, 이 탭의 활성 뷰
(`study.activeView.v1`)는 크로스탭 동기화가 없어 건드려지지 않는다 — 원래 탭의 URL 도
자기 `?view=` 를 들고 있으므로 새로고침 복원까지 그대로다. `study.workspace.v1` 의
write-through(§`state/studyWorkspace.ts`)는 사용자가 `/study` 를 손으로 두 번째 탭에 여는
경우에 **이미** 성립하던 성질이라, 이 진입점이 새로 만드는 위험이 아니다.

**범위는 행 하나다.** 그룹 헤더 ctrl+클릭(§5 가 지운 「그 종목의 최신 저장뷰」 분기),
행 메뉴 항목, 미들 클릭, ctrl+Enter 는 **넓히지 않았다** — 계산만 하고 버려지는 배선을
남기지 않는다는 §5 의 규율은 여기서도 같다.

**그래서 이 문서를 인용해 그 분기를 다시 지우지 말 것.** 「ADR-0149 로 사라졌다」고 적힌
주석이 코드에 여럿 남아 있고, 전부 *앱* 탭을 가리킨다. 판별식은 **어느 탭인가** 하나다.
