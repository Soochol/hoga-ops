...
**ADR-0157(2026-08-23)이 이 ADR 을 부분 supersede 한다** — `/study` 라우트가 폐지되면서
§7 의 딥링크 경로가 `/study?view=` → **`/live?view=`** 로 바뀌었다. **탭 스트립은 돌아오지
않는다는 판정과 「탭 = 브라우저 탭」이라는 어휘는 그대로다**(0157 이 같은 강도로 다시
못박는다). `study.activeView.v1` 은 0155 가 이미 승계 전용으로 내렸고, 이제 승계 대상도
없다.
**ADR-0155 (2026-08-21) 가 이 ADR 을 부분 supersede 한다** — "저장뷰는 한 번에 하나" 만
뒤집혔다(축이 탭에서 **창의 링크 그룹**으로 옮겨갔다). 탭 스트립 제거 판정과 §4·§5 의
"뷰포트 캡처·disposition 은 되살리지 않는다" 는 그대로다. 아래를 읽을 때 `study.activeView.v1`
은 **승계 전용 키**로 읽을 것 — 그 스토어는 삭제됐고 값은 `groupViews[1]` 로 옮겨갔다.
