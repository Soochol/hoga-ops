...
## Amendment (2026-07-08): 전제 반증 기록 — failover는 유지, 재검토 신호 갱신

**전제가 반증됐다.** 본 ADR의 Context는 ADR-0082의 "healthy 계좌를 추가하면 REST
용량이 대략 선형으로 늘어난다"는 약속을 **실패 경로에도 적용**하는 것을 목표로 삼았다.
그러나 그 약속의 기반 가정(계좌/appkey 단위 한도)은 2026-07-08 기준 반증됐다 —
한도는 명의 단위이고 모든 `KisClient`가 전역 토큰버킷 하나를 공유한다(ADR-0082
2026-07-08 amendment). 명의가 포화된 상태에서는 계좌 B로 failover해도 같은 전역
버킷에서 송신하므로, EGW00201이 명의-단위로 걸린 경우 failover가 요청을 구제하지
못할 수 있다.

**그럼에도 failover 루프는 유지한다.** 두 근거:

1. **발동 실적 0 = 비용 0.** 운영 백엔드 `/api/live/status`의
   `rate_limit_failovers=0` — 전역 버킷이 송신률을 15/s로 캡한 뒤로 계좌 내
   `KisClient` backoff(ADR-0050)가 순간 초과를 흡수해, failover 지점까지 도달하는
   `KisRateLimitError`가 사실상 발생하지 않는다. 루프는 40줄짜리 무해한 보험으로
   남는다.
2. **미래 케이스 대비.** KIS가 EGW00201을 명의-단위가 아니라 appkey-단위로도
   추적하는 케이스(예: 특정 엔드포인트의 계좌별 카운터)가 확인되면 failover가 다시
   유효해진다. 단일 계좌 구성에서는 `max_attempts==1`이라 어차피 no-op이다.

**재검토 신호(갱신).** `rate_limit_failovers`가 실제로 증가하기 시작하면 —
전역 버킷 아래에서 failover가 요청을 구제하고 있는지(성공률)와, 명의-단위 체제에서
계좌 간 재시도가 오히려 다른 계좌의 전역 버킷 예산을 낭비하는 건 아닌지를 실측으로
재평가한다. 증가가 관측되지 않으면(현 상태) 루프는 그대로 둔다.
