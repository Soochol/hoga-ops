...
3. **그룹 드래그 중에는 전 그룹을 헤더만 남기고 접는다.** `verticalListSortingStrategy`
   ...
   스크롤 없이 재배치 결과 전체가 보인다. dnd-kit 공식 sortable-tree 예제와 같은 수법.
   - **수치를 인용할 때 축을 맞출 것.** 위 `733px` 는 *세 슬롯* 이동에서 **블록**이 뛴
     거리이고(같은 드래그의 포인터는 330px 였다 — 둘이 어긋난 것이 애초의 결함이다),
     접은 뒤 실측한 `58px` 는 *두 슬롯* 이동에서 **포인터**가 간 거리다. 그대로 나란히
     쓰면 다른 것을 비교하게 된다. 같은 축으로 옮기면: 종전엔 그룹 하나를 지나는 동안
     ~500px 무반응 뒤 한 번에 점프했고, 지금은 **슬롯당 29px 1:1 추종**이며 칩·포인터·
     미리보기가 같은 좌표에 겹친다(실측: 칩 top 94 · 포인터 y 109 · 착지 슬롯 94~123).
