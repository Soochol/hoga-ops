...
> **부분 대체 (2026-09-08, v0.13.0.0):** 아래의 그룹 내부 전용 이동, 행·헤더 전체
> 드래그 표면, 종목 이동의 낙관적 저장 설명은 [관심종목 패널 이동 UX](../superpowers/specs/2026-09-08-watchlist-panel-transfer.md)로 대체한다.
> 현재는 종목·그룹 모두 전용 ⠿ 핸들에서 시작하며, 다중 선택과 그룹 간 정확한 위치 이동을
> 지원한다. 종목 이동·되돌리기는 `PUT /api/watchlist/items/transaction`으로 한 번에 검증·저장하고
> 성공 후 표시한다. 그룹 재정렬의 기존 저장 경로와 단일 DndContext·그룹 블록 구조는 유지한다.
> 아래 내용은 당시 결정과 측정 이력이다.
