...
- `close_ms=0` 129건은 미해결 — 수집 미완 복합 결함으로 별도 재캡처 트랙(후속 ADR).
  → **후속 ADR-0130 으로 종결(2026-07-29).** 그 "재캡처 트랙" 전제는 성립하지 않는 것으로
  판명됐다: hogaplay 보유가 ~18시간이라 대상 거래일의 원본이 업스트림에 이미 없고,
  받아지는 것은 `close_ms=0` 스텁(이벤트가 정상의 ~9%)이다. 복구 불가로 확정하고
  재시도만 차단했다. 그사이 129건 → 573건으로 늘어 있었는데, 그 증가분 대부분은 이
  클래스에 재시도 차단이 없어 백필이 반복 재생산한 결과였다.
