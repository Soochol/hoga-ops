...
3. **LRU 증설.** 과거 분봉 캐시 512→2048(250일×2종목+워밍 여러 종목 공존).
   *(amended 2026-07-08, PR #453)*: 사이징 상향에 더해 캐시 구조 자체가
   **코드-지역성 2단 LRU**((venue, code) 코드-LRU 아래 date-LRU, per-code 쿼터
   320)로 재작성됐다 — 전역 예산 초과 시 LRU **코드**의 oldest부터 축출하므로,
   관심종목 순환이 활성 코드의 딥스크롤 창을 밀어내는 churn은 총량과 무관하게
   구조적으로 차단된다. 2048은 총량 상한으로 유지.
