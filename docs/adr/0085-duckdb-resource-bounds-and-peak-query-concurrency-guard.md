...
**(b) 무엇보다, 남은 GIL 무게는 candles 에 있지 않다.** 같은 구간·같은 방식:
...
`hoga`·`sidecar` 가 candles 보다 **훨씬 더** 나쁘고, 완전 직렬(6×)조차 넘는다
(캐시·메모리 대역폭 경합이 얹힌다). candles 만 컬럼화해도 전체 그림은 바뀌지 않는다.

**결론: 현재 wire 계약 아래서 이 상한은 영구적이다.** 없애려면 candles 하나가 아니라
`/api/range` 응답 **전체**를 컬럼→직렬화 경로로 재설계해야 하고, 그건 이 리포가
조용한 필드 스트립 사고를 겪고 세운 계약 기계를 걷어내는 일이다. 성능 몇 백 ms 와
맞바꿀 문제가 아니므로, **하려면 별도 ADR 로 의도를 먼저 세울 것.**
