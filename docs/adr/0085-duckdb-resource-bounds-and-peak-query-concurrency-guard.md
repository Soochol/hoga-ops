...
### v3.4 — 무게를 **모드**로도 가른다. sidecar 전용 레인 2 (2026-08-11)

v3.2 가 이월했던 항목이다: sidecar 는 6-스레드 팽창이 **3.8×** 로 **부선형**인데
(candles 6.7× · hoga 6~8×) 같은 줄에 세워 두면 그 이득을 버린다. v3 의 무게 분리가
**일수**만 봤으므로, 여기에 **모드** 축을 더한다.

**부하** — v3 과 같은 22 동시지만 구성을 바꿨다: 무거운 것 **12**(`/study` 저장뷰
4개분 × 3모드) + 가벼운 것 10(하루짜리). v3 은 무거운 것 6 + 가벼운 것 16 이었는데,
레인이 가르는 대상이 `heavy:sidecar` 라 저장뷰 2개면 그게 2건뿐이라 **판별력이 없다.**
저장뷰 4개는 탭 워밍이 도는 실제 `/study` 모양이기도 하다. 전부 warm(콜드 sidecar 는
141초라 섞이면 그 trial 을 버려야 한다), 실서버 HTTP, 교대 대조 4라운드 median:

| 변형 | wall | **뷰완성** | h:hoga | h:sidecar | h:candles | l:hoga | l:sidecar | 평균 |
|---|---|---|---|---|---|---|---|---|
| 공유(v3) | 9.35s | 8.27s | 6.16s | 7.06s | 8.24s | 0.144s | 0.277s | 3.72s |
| **레인 2** | **8.46s** | **7.73s** | **4.96s** | 6.19s | **6.81s** | 0.163s | **0.233s** | **3.26s** |
| 레인 4 | 8.95s | 8.69s | 6.45s | **4.75s** | 8.65s | 0.239s | 0.317s | 3.54s |

**"뷰완성" 열을 새로 뒀다** — 저장뷰 하나의 3모드 중 **가장 느린 것**이다. 프론트
`isLoading` 이 OR 라 화면은 그때 뜨므로 사용자가 실제로 기다리는 값인데, 클래스별
중앙값만 보면 안 보인다(첫 판에서 "sidecar 는 빨라졌는데 화면은 그대로" 를 놓쳤다).

**레인 4는 재분배다.** sidecar 를 -33% 얻는 대신 뷰완성 +5% · candles +5% · 가벼운
hoga **+66%** 를 잃는다. 레인을 넓게 열수록 sidecar 가 제한 레인의 candles/hoga 와
GIL 을 나눠 갉아먹기 때문이고, v3 이 "상한을 모두에게 걸면 손해" 라고 적은 것의
거울상이다. **부선형은 sidecar 끼리일 때의 성질이지 GIL 을 안 쓴다는 뜻이 아니다** —
peak 만 polars 이고 depth 히트맵·1~2MB 응답 생성은 파이썬이다. 같은 이유로
**완전 면제(무제한)도 기각**됐다: v3 의 계기였던 "자연 상한 = 22" 실패가 sidecar
클래스에서 그대로 재현된다(워밍 + 멀티 창이면 sidecar 만 여러 벌 동시).

**레인 2가 실제 이득이다.** 가벼운 hoga 만 +14%(절대 **+19ms**, 관측 노이즈 범위
0.121~0.199s) 나빠지고 나머지 전 열이 7~19% 개선된다. 채택값이 공유 상한과 같은
**2**인 것은 우연이 아니다 — 이득의 정체는 "sidecar 를 더 돌린다" 가 아니라
**"서로를 막지 않는다"** 다. 두 상수를 하나로 합치면 안 되는 이유이기도 하다(근거가
다르고 다음 재측정에서 갈린다).

⚠ **단발 sweep 이 이 결론을 뒤집었다.** 3-trial 1회에서는 레인 4가 최선으로 보였고
(wall 8.30s · 평균 3.165s), 교대 대조로 다시 재자 뒤집혔다. 변형 간 차이가 머신 부하
드리프트와 같은 크기라 **번갈아 돌리지 않으면 순서가 곧 결과다.** v3 의 "균일 부하로
잰 결론은 혼합 부하에서 뒤집힌다" 에 이어, 이번에는 **측정 순서**가 같은 일을 했다.

**이 분리가 못 보는 것.** (1) 레인 안은 여전히 FIFO 라 5개월과 2년이 섞이면 그 안에서
head-of-line 이 재현된다(v3 과 동일). (2) 모드가 셋인데 레인은 둘이다 — candles 가
가장 초선형(6.7×)인데 hoga 와 같은 줄에 있다. 뷰완성을 지배하는 것이 candles 라
다음 재측정은 그쪽을 가르는 쪽이 후보다. (3) 좁은 요청은 모드와 무관하게 그냥
지나간다 — 모드 분리는 일수 분리를 **대체하지 않는다**(테스트가 못 박는다).

### v3.5 — candles 도 자기 레인으로. **"초선형이니 좁게" 는 반증됐다** (2026-08-11)

v3.4 의 "못 보는 것 (2)" 를 해소한다. 뷰완성(저장뷰 3모드 중 가장 느린 것)의 지배자가
candles 였는데, 그것이 hoga 와 한 줄에 있었다.

**가설이 틀렸다.** candles 는 셋 중 **가장 초선형**이라(6.7× · 12스레드 N 대비 1.56)
"레인은 좁을수록 좋다" 로 예상하고 레인 1부터 쟀다. 그런데 candles 가 **더 느려졌다**
(7.11 → 7.46). 같은 부하·같은 절차, 교대 대조 3라운드 median:

| 변형 | wall | **뷰완성** | h:hoga | h:sidecar | h:candles | l:hoga | 평균 |
|---|---|---|---|---|---|---|---|
| 공유(v3.4) | 9.54s | 8.56s | 5.47s | 7.10s | 7.32s | 0.230s | 3.65s |
| 레인 1 | 9.61s | 8.42s | 2.06s | 7.36s | **7.46s** | 0.240s | 3.18s |
| 레인 2 | 10.03s | 7.93s | 2.19s | 7.89s | 4.82s | 0.277s | 3.07s |
| **레인 3** | 9.84s | **7.85s** | 2.29s | 7.79s | 2.88s | 0.269s | 2.83s |
| 레인 4 | 9.77s | 7.88s | 2.26s | 7.88s | 2.69s | 0.197s | 2.45s |

**초선형은 총 wall 의 성질이지 개별 요청 지연의 성질이 아니다.** 레인 1은 4건을
직렬화해 마지막의 대기를 누적시키고, 그 누적이 초선형 페널티보다 크다. 사용자가
기다리는 것은 총 wall 이 아니라 자기 요청이므로 이쪽이 이긴다. v3 이 "균일 부하로 잰
결론은 혼합에서 뒤집힌다", v3.4 가 "측정 순서가 곧 결과" 였다면, 여기서는 **어떤 값을
목표로 삼았는가가 결론을 정했다.**

**레인 3에서 꺾인다.** 4로 넓혀도 뷰완성이 안 움직인다(7.85 ↔ 7.88, 노이즈). 이유가
표에 그대로 있다 — 채택 변형의 뷰완성 7.85 는 `h:sidecar` 7.79 와 **같은 값**이다.
병목이 candles 에서 sidecar 로 옮겨갔고, candles 를 더 여는 것은 이제 GIL 만 더 나눈다.
그래서 셋 중 가장 좁은 3을 택한다(총 동시 계산 7 vs 8).

**공짜가 아니다.** `wall +3%` · `h:sidecar +10%` 는 일관되게 악화된다 — 병목 이동의
대가다. 그래도 랜딩하는 근거는 **뷰완성 −8%** 와 전체 평균 −23% 이고, wall 은
"마지막 요청까지" 라 사용자가 기다리는 값이 아니다. light 열은 라운드마다 부호가
뒤집혀(−14% ↔ +17%) 노이즈로 판정했다.

**구현** — 모드→레인이 표가 됐다(`mode_lanes`). if 사다리로 두면 모드를 가를 때마다
분기가 늘지만 표는 줄이 하나 는다. 공유 레인에 남은 모드는 이제 **hoga 하나**이고,
`RANGE_COMPUTE_CONCURRENCY` 의 이름은 이력으로만 남았다(상수 주석에 명시).

**이 분리가 못 보는 것.** (1) **병목은 이제 sidecar 다** — v3.4 가 레인 2로 정한 값은
candles 가 앞을 막던 시절의 최적이므로, 다음 재측정은 그 값부터 다시 볼 것.
**→ v3.6 에서 재측정했고 2를 유지한다.**
(2) 레인 안은 여전히 FIFO(v3·v3.4 와 동일). (3) 총 동시 계산이 4 → 7 로 늘었다 —
limiter 밖 엔드포인트가 굶는 현상(#1293 조사의 13.9s `past-daily-candles`)이 다시
관측되면 여기부터 의심할 것.

### v3.6 — sidecar 레인 **재측정: 2를 유지한다** (2026-08-11)

v3.5 의 "못 보는 것 (1)" 을 해소한다. candles 가 자기 레인으로 빠지면서 병목이
sidecar 로 옮겨갔고, v3.4 가 정한 레인 2는 **candles 가 공유 레인에서 앞을 막던
시절의 최적**이었으므로 조건이 달라진 지금 다시 쟀다. 다른 노브는 랜딩값 고정
(공유 2 · candles 3), 같은 부하·절차, 교대 대조 3라운드 median:

| 변형 | wall | 뷰완성 | h:hoga | h:sidecar | h:candles | l:hoga | l:sidecar | 평균 |
|---|---|---|---|---|---|---|---|---|
| **sc2 (현행)** | 9.91s | 8.00s | 2.42s | 7.97s | **3.01s** | 0.282s | 0.368s | **2.720s** |
| sc3 | 9.44s | 8.33s | 2.56s | 7.79s | 4.55s | 0.205s | 0.318s | 2.873s |
| sc4 | 9.51s | **7.53s** | 2.81s | **6.14s** | 5.12s | 0.249s | 0.392s | 2.718s |

- **sc3 은 전 열에서 나쁘다** — 뷰완성 +4% · candles +51% · 평균 +6%.
- **sc4 는 순수 재분배다.** 뷰완성 −6% · sidecar −23% 를 얻지만 **candles +70%**
  (3.01 → 5.12s) · hoga +16% 를 잃고, **전체 평균이 2.720 → 2.718 로 사실상 동일**하다.
  v3 이 세운 기준에 정확히 걸린다 — *"총 wall 이 같으면 상한은 이득이 아니라 재분배"*.
  뷰완성 차이도 raw 가 겹친다(sc2 `[8.29, 8.00, 7.98]` ↔ sc4 `[7.37, 7.53, 7.93]`).

**왜 이번엔 이득이 없나 — 이 절의 핵심.** v3.4 에서 sidecar 분리가 이득이었던 이유는
**candles 와 한 줄에 있던 것을 뗀 것**이지 "sidecar 를 더 돌린 것" 이 아니었다(채택값이
공유 상한과 같은 **2**였다는 사실이 그 증거다). 그 분리는 이미 끝났다. 지금 레인을 더
여는 것은 없던 줄을 만드는 게 아니라 **같은 GIL 을 더 크게 가져오는 것**이라, 다른
레인에서 그만큼 나온다. 총 동시 계산이 이미 7(2+2+3)이고 sc4 면 9인데, 32코어라도
GIL 은 하나다.

**따라서 레인 폭 조정은 여기서 수확이 끝났다.** 뷰완성을 더 줄이려면 다음은 폭이
아니라 **compute 비용** 쪽이다 — 콜드 캐시 예열(#1293 의 남은 항목), candles 의 행→
pydantic 모델 생성(v3.1 이 wire 계약 문제로 보류한 자리).

⚠ 재지 않은 조합: `sc4 + candles 축소`(총량 유지). candles 레인 2는 v3.5 에서 이미
열등했고, 두 축을 동시에 움직이면 변형 수가 곱으로 늘어 교대 대조 비용이 급증한다.
