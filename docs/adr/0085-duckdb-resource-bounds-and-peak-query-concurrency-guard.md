...
**함의 둘.** (1) `gc.freeze()`(기동 후 상주 객체를 영구 세대로) + `gc.set_threshold`
상향은 코드 구조를 안 건드리고 이 몫을 겨냥한다 — polars 재설계보다 훨씬 싸다.
다만 순환 참조 회수가 늦어지므로 RSS 를 함께 관측해야 하고, 이 ADR 이 애초에 OOM
사고에서 태어났으므로 **가볍게 넣을 변경이 아니다**. (2) sidecar 가 부선형이라는
사실은 무게 분리 기준(요청 **일수**)이 최적이 아닐 수 있음을 시사한다 — 비용이
아니라 **모드**로 가르면 sidecar 는 상한 밖에 둘 수 있다. 둘 다 이 PR 범위 밖이며,
근거만 남긴다.
