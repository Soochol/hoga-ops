...
⚠ **위 표와 "1" 이라는 결론은 이 시점의 것이다.** 상한 값은 compute 비용의 함수라
그 뒤로 또 움직였다 — 최신 값과 근거는 **v3.3** 을 볼 것. 이 시점의 경위만 적어
두면: `model_copy` 로 행을 두 벌 만들던 시절에는 compute 가 2배여서 **2**가
최적이었고, `ts_ms` 보정을 SQL 로 민 뒤 대기가 싸져 **1**이 이겼다.
...
**상한 재측정 — 값이 다시 2로 돌아갔다.** 상한은 compute 비용의 함수이고 그 비용이
또 줄었으므로 혼합 부하 sweep 을 다시 돌렸다(실서버 HTTP, 3회 median):

| 상한 | wall | 가벼운것 중앙 | 무거운것 중앙 | 전체 평균 |
|---|---|---|---|---|
| 무제한 | 1.46s | 0.30s | 1.33s | 0.56s |
| 1 | 1.57s | 0.14s | 1.06s | 0.38s |
| **2** | **1.32s** | **0.14s** | **0.98s** | **0.34s** |
| 4 | 1.44s | 0.20s | 1.16s | 0.42s |

3회 모두 2가 우세했다(평균 0.36/0.30/0.34 vs 1의 0.38/0.40/0.37). GC 오버헤드가
빠지자 동시 실행의 손해가 줄어 병렬 이득이 살아난 것이다. `RANGE_COMPUTE_CONCURRENCY`
를 1 → **2** 로 되돌렸다.

이로써 이 값은 **세 번 움직였다**: `model_copy` 시절 2 → `ts_ms` SQL 이관 후 1 →
GC 튜닝 후 다시 2. **성능 개선과 큐 정책은 독립이 아니다** — 이 경로의 비용을
바꾸는 변경은 반드시 이 sweep 을 동반해야 한다.

**GC 튜닝의 실서버 효과**(같은 sweep, `HOGA_GC_GEN0_THRESHOLD=0` 대조군 대비):
상한 2 기준 wall 2.09s → 1.32s, 전체 평균 0.59s → 0.34s. 프로브의 +37~55% 보다
작게 보이는 구간도 있는데, HTTP 응답에는 직렬화·전송 등 GC 와 무관한 비용이
섞이기 때문이다.
