...
| 측정 | 값 |
|---|---|
| `build_range_bundle(candles)` 98일·36,276봉 단독 | 0.63s |
| 조용한 서버에서 같은 요청 HTTP 왕복 | 0.63s |
| 서버 slow-log median (동시 22건 구간) | candles 8.9s · hoga 8.3s · sidecar 14.6s |
| 같은 로그 max | sidecar 213.2s |
| 스레드 6개 동시 (32코어) | wall 5.9× — 커넥션 개별 지급 시에도 5.2× |

`build_candles_slice` 내부는 DuckDB `query_all` 60% · `model_copy` 재타임스탬프
38.7% · 파일시스템 1.4% 다. parquet 이 날짜당 10KB 라 쿼리 실행 자체는 미미하고,
시간의 대부분이 모델 객체 생성이다(36,276개를 만들고 다시 36,276개).
...
| 1 | 2.28s | **2.26s** | 1.43s | 2.01s |
| 2 | 2.24s | 1.89s | 1.59s | 1.77s |
| 4 | 2.06s | 1.69s | 1.56s | 1.63s |

가벼운 요청의 중앙값이 **7배** 나빠진다. 총 wall 은 어느 쪽이든 같으므로 상한은
...
| **2** | **1.89s** | **0.12s** | **1.38s** | **0.44s** |
| 4 | 1.87s | 0.12s | 1.43s | 0.48s |

**모든 열이 개선된다** — 재분배가 아니라 실제 이득이다. 값이 2인 이유: 1은 넓은
것끼리도 완전 직렬이라 그쪽 중앙값이 도로 올라가고(1.43), 4는 GIL 경합이 다시
붙어 이득이 없다. 30일 경계는 두 사용처 사이의 빈 구간이며, 틀려도 실패하지
않는다(좁은 것이 상한을 타거나 넓은 것이 안 타는 것뿐).
...
워커를 늘리면(README 가 금지한다) 상한도 그만큼 배수가 된다. (4) 근본 해결이
아니다: `ts_ms` 보정을 SQL 로 밀어 모델을 두 벌 만들지 않게 하거나, 이 경로도 v2
처럼 컬럼 지향으로 옮기면 GIL 이 풀려 상한 자체가 불필요해진다. 그때 이 절을 다시
열 것.
