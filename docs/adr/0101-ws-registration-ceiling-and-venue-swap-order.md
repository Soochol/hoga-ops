...
> ⚠ **아래 세 항목은 ADR-0136(KIS 제거, 키움 전면 대체)으로 무의미해졌다 —
> 재착수 대상이 아니다** (2026-08-29 확인). 전부 **KIS WS** 를 전제하는데 그 구현이
> 이제 없다: 지목된 `hoga/live/ws_client.py`·`ws_fields.py` 는 **존재하지 않고**,
> "연결당 41 등록" 상한도 코드에 없다. 현행은 키움이고 상한은
> `hoga/live/coverage.py` 의 `KIWOOM_WS_MAX_REGISTRATIONS = 200`(계정당)이다.
> 등록 수를 줄이는 현재의 열린 질문은 여기가 아니라 **ADR-0116 의 `_AL` 통합
> 스트림**(정규장에서 KRX·NXT 를 한 스트림으로 받을 수 있는가)이다.
