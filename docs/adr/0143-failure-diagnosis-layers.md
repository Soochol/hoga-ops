...
| Phase 3-D | 동시 발생 조율 → **보류 판정** | (문서) |
| Phase 3-C | 심각도 축 | 미착수 |
| Phase 4 | 렌더 채널 규약 | 미착수 |
...
**진행 상태**: A 는 `transport` 를 통일했다("연결 실패" → "연결 불가", ADR-0137 어휘).
`rate_limit` 은 토스트("시세 서버 혼잡" — 상태)와 스크리너("호출 한도 초과" — 원인)가
갈려 있는데, **어느 쪽이 옳은지가 어휘 취향이 아니라 정보량 판단**이라 미결로 둔다.
