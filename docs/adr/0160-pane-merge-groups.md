...
10. **UI 진입점은 왼쪽 레전드 이름이다** — 드래그(pane 본체 = 병합, 경계 = 이동·분리,
    6px 임계값, Esc 취소)와 클릭 메뉴(비드래그 폴백: 위·아래 이동/병합/분리/축 모드). 드롭 존
    지오메트리는 레전드 Y 배치와 같은 소스(`chart.panes()` 누적)라 화면과 어긋날 수
    없다. **2026-09-07 갱신:** 우측 이름·그립·↑/↓는 제거하고 모든 보조지표 pane의
    왼쪽 이름으로 통합했다. 값 레전드가 없는 pane도 이름을 제공하며, 메뉴의 위·아래
    이동은 그룹 전체에 적용한다. ✕는 기존 역할을 유지한다. 높이가 낮은 병합 pane은
    이름 위 스크롤이나 Tab으로 아래 멤버에 접근한다. 접기(`foldPanes`)의 원자 단위는 그룹이다.
