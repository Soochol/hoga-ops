...
**⚠ 함정 ②: 일별 축 + 코스닥의 `kospi200`·`basis` 는 틀린 값이다** (2026-08-05 추가 실측).
같은 날짜를 두 시장으로 조회하면 **과거일 값이 완전히 동일**하다:

```
ka90010  KOSPI  08/04 kospi200 1000.03  basis  -0.03    08/03 986.72  basis 5.63
ka90010  KOSDAQ 08/04 kospi200 1000.03  basis 345.27    08/03 986.72  basis 275.48
                        ^^^^^^^ 동일               ^^^^^^ 불가능한 크기
```

장중 축(ka90005)은 **시장별로 옳다** — 같은 시각 코스피 1046 / 코스닥 1361(≈KOSDAQ150),
basis 둘 다 2~3. 즉 같은 이름의 열이 두 TR 에서 의미가 다르고, 일별 축에서는 코스닥
쪽이 신뢰 불가다. 배선은 그 자리만 `None` 으로 접는다(`trust_index_columns=False`) —
순매수 3열은 코스닥 일별에서도 정상이라 그대로 쓴다.
...
## 2-d. ka90005·ka90010·ka10131 금액 단위 = **백만원** (2026-08-05 추가 실측)

세 TR 모두 `amt_qty_tp` 를 0/1 로 뒤집어도 **응답이 동일**하다 — 파라미터가 무시되고
금액·수량 양축이 한 응답에 온다(ka10131 은 `_amt`/`_qty` 필드 분리, ka90005/90010 은
`*_netprps` + `*_netprps_qty`).

**단위 판별은 함의 주가다**: ka10131 NAVER 금액 `129,866` ÷ 수량 `609,980주` =
**212,900원/주 = 실주가와 일치** → 금액은 백만원. 억원이면 주당 2,129만원으로 불가능.
ka90005 도 같은 단위(당일 프로그램 전체 ~1.09조 = 코스피 회전 13.5조의 8% 로 타당,
억원이면 109조로 불가능).

**⚠ 이 페이지에서만 단위 오표기가 세 번째다** — ka10064 백만원 · ka10051 **억원** ·
ka90005/ka10131 백만원. `amt_qty_tp` 표와 단위가 **TR 마다 전부 다르다**. 배선은
백만원 → 억원으로 정규화하고 필드 이름에 단위를 박는다(`*_eok`).

실화면 증상(수정 전): 프로그램 코스피 `+1,083,226` 을 "억원" 라벨로 노출 → 108조로
읽힘. 순매수 NAVER `+129,866억` = 13조. 라벨이 아니라 **값의 자릿수가 소리를 낸다** —
단위 의심 시 함의 주가·회전율 대비로 즉시 판별할 것.
