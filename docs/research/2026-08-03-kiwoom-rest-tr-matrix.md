...
|---|---|
| 대응 확인 | **10 / 11** |
| 갭 | **1** — **휴장일**뿐 (투자자 추정은 #1013 에서 `ka10064` 로 해소, 아래 정정) |
...
| 9 | 투자자 매매동향 **추정** | `HHPTJ04160200` | **`ka10064`** | `/api/dostk/chart` | 시각별 | 9 | `tm` 슬롯 시계열 | ✅ **대응** (2026-08-03 정정) |
...
### 9. 투자자 매매동향 추정 — ✅ `ka10064` (2026-08-03 정정)

> **정정**: 이 문서의 초판은 이 표면을 **갭**으로 기록했다. **틀렸다.** #1013 에서 `ka10064`(장중투자자별매매차트)를 찾아 실측했다. 초판이 놓친 이유는 카탈로그 질의를 "투자자 매매" 로만 했고 **"장중"** 축으로 다시 묻지 않아서다 — 일별·합계 TR 만 회수됐다.

```
/api/dostk/chart  api-id ka10064
body {"stk_cd":"005930","mrkt_tp":"000","amt_qty_tp":"1","trde_tp":"0"}
→ 래퍼 키 opmr_invsr_trde_chart, 9필드, 30~45ms
```

KIS `HHPTJ04160200` 과의 필드 대응:

| KIS | 키움 `ka10064` | 실측값 예 |
|---|---|---|
| `bsop_hour_gb` (슬롯) | **`tm`** | `090000` · `091900` · `095700` · `110200` |
| `frgn_fake_ntby_qty` (외국인 가집계) | **`frgnr_invsr`** | `-38541` → `-62603` → `-164288` |
| `orgn_fake_ntby_qty` (기관 가집계) | **`orgn`** | `-173941` → `-233992` |
| — | `invtrt`·`insrnc`·`bank`·`penfnd_etc`·`natn`·`etc_corp` | 기관 6종 분해 |

슬롯 간격이 불규칙한 것(`090000`→`091900`→`095700`)은 KIS 가집계 발표 주기와 같은 성격이다. **키움이 오히려 더 세분화**돼 있다.

관련: `ka10063`(장중투자자별매매, `/api/dostk/mrkcond`, 34행/21필드 — **종목 리스트**라 모양이 다르다) · `ka10131`(기관외국인연속매매현황 — 연속 순매수 일수라 성격이 다르다).
...
| 갭(**휴장일**) 처분 | [#1013](https://github.com/Soochol/hoga-ops/issues/1013) — 투자자 추정은 `ka10064` 로 해소됨 |
