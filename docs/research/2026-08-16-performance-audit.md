...
~~**착수 전 선행 조건 (코드 수정 아님).** 장중 1회 계측…~~ — ✅ **2026-09-04 계측 완료**
(`2026-09-04-intraday-flow-measurement.md`). 결과: 종목당 **17~19 ob/s**(대형주) ·
**4~6**(중형주)이고, 15분 보존창에서 `live.ob.length` 가 **대형주 16,155 · 중형주 4,248**
이다. 판정 기준(「수천 급이면 트랙 1 최상위」)을 **넘는다** — 대형주 한 종목이
안전핀(`MAX_BUFFER_PER_KIND = 60_000`)의 27%를 혼자 쓴다.
