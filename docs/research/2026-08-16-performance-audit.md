...
| 6 | ✅ **구현됨** 슬라이딩 축출이 prefix-guard 를 깸 | A-1 · B-1 · E-1 | CONFIRMED×3 | flush 당 **8~13배**(실측) | **M/L** | 중간 |
...
> 함수(`:372`)·호출부(`:903`)·테스트(`range.test.tsx:2264`)가 있고, 그 주석은
> "올리는 것은 성능 감사의 결론을 되무는 일" 이라며 이 표를 되가리킨다. 즉 **표에
> 남은 진짜 미착수는 #11 하나다.**
>
> 이 표를 "다음에 뭘 할까" 의 입력으로 쓰는 사람에게 stale 마커는 곧 잘못된
> 우선순위다. 항목을 랜딩할 때 **같은 PR 에서 마커를 붙이는 것**이 그 재발을 막는다.
...
> **✅ 구현 완료 (2026-08-17).** 아래 선행 조건은 **충족되지 않은 채** 착수했다 — 정당한가?
> 착수 직전에 **비용 분해**를 새로 재서 그 판단을 대신했다: 축출 시 비용의 **88~90% 가
> 재소비**이고 그 비율이 900~18,000 전 구간에서 안정적이다(classify 는 `pushTopK` 조기
> 종료 + 이진탐색이라 싸다). 즉 **N 은 「고칠 가치가 있는가」가 아니라 「얼마나 아꼈는가」를
> 정한다** — 어느 N 이든 이 수정은 같은 비율로 이득이다. 그래서 계측을 기다리지 않았다.
> 실측(같은 벤치·같은 픽스처): 900 1.66→0.20 · 4,500 8.62→0.69 · 9,000 18.39→1.62 ·
> 18,000 45.00→3.42 ms (**8~13배**).
>
> **아래 「기계적 이식은 아니다」 경고는 그대로 지켜졌다** — 슬라이딩과 축출을 같이 넣었고
> (`eventSeq` prefix 절단 + `head` 포인터 + trade 스냅샷별 터치 기여 개수), 판정이 모호해지는
> 입력(t_ms 중복·터치 역순)에는 **sticky 플래그로 전량 재소비 폴백**을 뒀다.
> 가드는 고정 시드 오라클 파리티(ask/bid × 일반/cutoff, 800스텝)이고 **네 가드 전부 red-check**
> 을 확인했다. 장중 `live.ob.length` 계측은 이제 **절감량 확인용**으로만 남는다.

~~**착수 전 선행 조건 (코드 수정 아님).** 장중 1회 계측…~~ — **이 조건은 위 구현
블록이 이미 무효화했다**(착수가 계측을 기다리지 않았고, 그 판단의 근거를 거기 적었다).
2026-09-04 에 그 계측을 **절감량 확인용으로** 수행했다
(`2026-09-04-intraday-flow-measurement.md`):

종목당 **17~19 ob/s**(대형주) · 4~6(중형주) · 0.8(소형주). 15분 보존창에서
`live.ob.length` 가 **대형주 16,155 · 중형주 4,248** 이다. 즉 실제 유량은 위 표의
**18,000 행(가장 비싼 구간)에 대형주가 실제로 앉아 있고**, 이 항목이 거기서 8~13배를
아꼈다는 뜻이다. 「1~5 ob/s 일 수 있다」는 보수적 가정은 **대형주에 한해 틀렸다**.
...
> 하려면 별도 헬퍼가 필요하고, 그건 이 항목 고유의 작업이다.
>
> **✅ 구현 완료 (2026-08-17).** 그런데 필요한 것은 **2행이 아니라 1행**이었다 —
> 당일 경로는 `intraday_price` 가 `basis_row` 를 단락시키므로 `rows` 를 쓰는 곳이
> `previous_row` 하나뿐이고, `no_basis_bar` 는 그 경로에서 **도달 불가**다. 폴백
> (`_latest_available_basis`)도 `intraday_prices is None` 안에만 있어 당일에는 안 탄다.
> 그래서 **캐시를 손대지 않고 읽는 양만 줄였다**(위 「캐시 축 분리」 처방보다 안전 —
> 메모된 dict 공유·변형·스테일 표면이 아예 없다). 실측 **423→23 ms(18.4배)**,
> `previous_row` 296종목 전수 일치. 과거 날짜 경로는 **그대로 두었다.** **아래 캐시 축 분리는 3-1 을 하고도
> 남는 비용이 측정될 때만 착수한다.**
...
2. **오늘(2026-08-16)은 토요일이고 dev·워크트리 백엔드는 무자격이다.** `GET /api/live/status` 의 `live_buffer.published_total` 이 0 이었다. 따라서 **장중 유량(ob/s)은 이 감사에서 한 번도 측정되지 않았다** — 트랙 1-3 의 12~310 ms 범위가 어디에 떨어지는지는 미확정이고, 착수 전 1회 계측이 선행 조건이다. 리포 자신의 메모리도 "키움 WS 틱 주기 미실측" 을 남겨 두었다.
   > ✅ **2026-09-04(금) 장중에 쟀다** — `2026-09-04-intraday-flow-measurement.md`.
   > 대형주 **17~19 ob/s**, 중형주 4~6, 소형주 0.8. 즉 트랙 1-3 의 범위는 **대형주에서
   > 상단(125~310 ms/flush)** 에 떨어진다. 이 문서가 인용한 옛 WS 실측(18.5/s)과
   > 삼성전자 값이 독립적으로 일치했다.
   > ⚠ 그 계측은 **순진한 방법 셋이 전부 틀린 답을 낸다** — `per_kind` 는 누적이 아니라
   > 보유량이고(음수가 나온다), 보유량÷retention 은 큐 상한(360) 때문에 못 쓰고,
   > `published_total` 은 kind 별로 안 나뉜다. 되는 방법은 **360개가 덮는 시간 범위**다.
