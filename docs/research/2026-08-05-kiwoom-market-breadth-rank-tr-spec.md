...
미확인: `dt` 의 의미(연속 최소일수 필터인지 조회 기간인지)와 허용값, `netslmt_tp` 코드표
(2=순매수로 추정). 배선 전에 값을 흔들어 확인할 것.
...
- **파라미터 코드표 미확정** — ka10131 `dt`/`netslmt_tp`, ka10034 `dt`, ka10016 `ntl_tp`/`high_low_close_tp`. 배선(#1101) 전에 값을 흔들어 확정할 것.
