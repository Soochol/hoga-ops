...
### `mrkt_tp` 코드표 — **확정 + 조용한 폴백 주의** (2026-08-10 실측, 4콜)

`001` = 코스피 · `101` = 코스닥. 반환 종목을 커밋된 심볼 마스터로 판정했다 —
`101` 은 100행 **전부 코스닥**이고 코스피와 교집합 0이다.

**⚠ 모르는 코드가 거절되지 않는다.** `1` · `000` 은 HTTP 200 · `return_code=0` 으로
**코스피 100행을 그대로** 돌려준다. 즉 이 TR 에서 시장 코드를 잘못 쓰면 증상이 에러도
빈 응답도 아니라 **틀린 시장 데이터**다. 다른 TR 의 코드표(`ka20003` 은 `0`/`1`,
`ka90005` 는 `P00101`/`P10102`)를 그대로 옮겨 쓰면 정확히 이 함정에 빠진다.

배선(#1249)이 이걸 화면 라벨(`MarketName` = `KOSPI`/`KOSDAQ`)만 wire 로 받고 벤더
코드는 라우트 내부에서 매핑하는 근거로 삼았다 — 원시 코드가 오가는 표면 자체를
만들지 않는다.
