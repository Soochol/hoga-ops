...
def test_120m_and_240m_have_no_vendor_scope_and_must_not_get_one() -> None:
    """표시는 하되(#1253) 이 맵에는 **넣지 않는다** — 두 가지 이유가 겹친다.

    ① 벤더가 안 준다(`ka10080` 은 1/3/5/10/15/30/45/60).
    ② 받아도 못 쓴다. 두 tf 는 정규장 마감(15:30)을 가로지르는 폭이라 벤더 120m 봉
       `[15:00,17:00)` 은 **이미 정규장+시간외 혼합**이고, 봉 단위로 클립할 방법이
       없다. 그래서 프론트 `fetchBucketMsFor` 가 30m 로 받아(15:30 이 봉 경계로
       남는다) 클립한 뒤 ×4·×8 로 접는다.

    즉 여기에 값을 넣는 것은 "덜 된 것을 채우는" 일이 아니라 클립을 **무력화하는**
    일이다 — 넣으면 혼합 봉이 그대로 화면에 온다.
    """
