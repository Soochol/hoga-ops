...
async def test_fetch_sends_the_adjust_flag(adjust: bool, want: str) -> None:
    ...
    c = _client(_h)
    await fetch_daily_candles(c, "005930", "20260803", "20260803", adjust=adjust)
...
async def test_rows_older_than_from_are_not_violations() -> None:
    ...
    res = await fetch_daily_candles(c, "005930", "20260731", "20260803")
    assert [x.reason for x in res.violations] == []
    assert len(res.candles) == 2
    await c.aclose()


async def test_rows_newer_than_to_are_still_violations() -> None:
    """반대 방향은 진짜 이상이다 — `base_dt=to` 를 줬는데 미래가 왔다."""
    c = _client(lambda _r: _ok([{**ROW, "dt": "20261231"}, {**ROW, "dt": "20200101"}]))
    res = await fetch_daily_candles(c, "005930", "20260803", "20260803")
...
async def test_walk_stops_once_the_page_reaches_past_from() -> None:
    ...
    c = _client(_h)
    await fetch_daily_candles(c, "005930", "20260701", "20260803")
...
async def test_truncation_is_never_silent() -> None:
    ...
    res = await fetch_daily_candles(c, "005930", "19900101", "20260803")
...
async def test_fetch_threads_the_venue_suffix() -> None:
    ...
    c = _client(_h)
    for v in ("KRX", "NXT", "UN"):
        await fetch_daily_candles(c, "005930", "20260803", "20260803", venue=v)  # type: ignore[arg-type]
