...
def test_ohlc_from_tick_prices():
    ...
    bar = _seal_all(agg)[("005930", "KRX")][M0].payload
...
def test_flush_seals_only_past_minutes():
    ...
    stamps = {s.t_ms for s in out[("005930", "KRX")]}
    assert stamps == {M0}
...
def test_flush_does_not_remove_until_commit():
    ...
    assert first[("005930", "KRX")][0].t_ms == M0
    # commit 전이라 다시 flush하면 같은 봉이 또 나온다(durability — append 실패 시 롤).
    again = agg.flush(now_ms=M1 + 5_000)
    assert again[("005930", "KRX")][0].t_ms == M0
    # commit 후엔 사라진다.
    agg.commit("005930", "KRX", minutes=[M0 // MIN])
...
def test_multiple_minutes_sealed_in_order():
    ...
    assert [s.t_ms for s in out[("005930", "KRX")]] == [M0, M1]
...
def test_set_active_codes_evicts_untracked():
    ...
    out = _seal_all(agg)
    # 멤버십은 **종목 단위** — 키가 (code, venue) 로 바뀌어도 축출 판정은 코드로 한다.
    assert set(out) == {("005930", "KRX")}
...
def test_multiple_codes_isolated():
    ...
    out = _seal_all(agg)
    assert out[("005930", "KRX")][M0].payload["close"] == 100
    assert out[("000660", "KRX")][M0].payload["close"] == 200
    assert out[("000660", "KRX")][M0].payload["volume"] == 7
