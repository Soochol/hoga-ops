...
- **구분선 최소화 — "톤 밴드" (2026-07-22, /live 프로토타입 4시안 비교로 C안 채택):** 크롬
  ...
  기능적 테두리(입력·팝오버·드롭다운·현재가 강조 박스·활성 스파인 `border-l-2`).
  *(여기 있던 "chart pane separator(드래그 리사이즈 기능 보유)" 는 2026-08-07 삭제 — 앱에
  드래그 스플리터가 더는 없다. 위 Content framing 항목 참조.)* 창 이음매(2px gap+그림자)는 카드 모델의 일부로 존치 —
