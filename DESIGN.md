...
- **Live workspace 창 경계 (2026-09-09, 사용자 승인):** 모든 `/live` 창은 필드 배경(`bg-bg`)과 둥근 모서리를 유지하고 **1px 중성 외곽선 + 짧은 그림자**로 겹친 창을 구분한다. 기본 선은 `--window-border`(`--border-strong`), 활성 창은 `--window-border-active`(`--accent`)와 기존 헤더 틴트를 쓴다. 안착 그림자 `--window-shadow`는 다크 `0 2px 8px rgba(0,0,0,.28)`, 라이트 `0 2px 8px rgba(30,39,50,.12)`이며 이동 중에는 `--shadow-modal`로 교체한다. 외곽선은 카드 내부의 포인터를 받지 않는 장식 레이어여서 창 rect·콘텐츠 크기·2px 간격·리사이즈 핸들은 그대로다. 그룹 색상은 배지에만 쓴다. 과거 borderless 정책에서는 겹친 창의 헤더가 가려질 때 경계를 읽기 어려워 이를 개정했다. **2026-08-09에 삭제한 2px 리사이즈 호버 accent 막대는 복원하지 않는다.** 공유 코어에서는 `outlined` opt-in으로 제공한다. `/study` 경로는 현재 `/live`로 리다이렉트된다.
