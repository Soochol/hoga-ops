...
- 🚫 **타이포에는 임의값 px 를 쓰지 않는다** (`text-[10px]` 류). 다이얼이 못 잡는 유일한
  DOM 표면이라 그 텍스트만 밀도 변경에서 낙오된다. 크기가 애매하면 **가장 가까운 토큰으로
  스냅**할 것 — 새 토큰을 만들지 말고. (2026-08-04 에 70건/31파일을 정리했다. `10.5px`·
  `11.5px`·`13px` 같은 값이 특히 함정인데, DESIGN.md 표의 **base-intent 열**을 그대로 px 로
  박은 흔적이라 "토큰과 같은 값" 처럼 보이지만 실제 렌더는 ×1.125 된 값이다.)
...
|---|---|---|---|
| `badge` | 8.5px | 9.563px | Micro-labels: status chips, markers, count badges, shortcut hints |
...
| 2026-08-04 | **타이포 임의값 px 70건/31파일 → 토큰 클래스 스냅** (`badge`·`xs`·`sm`) — 다이얼 밖 텍스트 소멸 | `text-[10px]` 류는 `:root font-size` 다이얼이 못 잡는 유일한 DOM 표면이라, 미래 Compact/Cozy 토글에서 그 텍스트만 낙오된다. **매핑은 rendered 기준 최근접 스냅**: `9`·`9.5`·`10px`→`badge`(9.563) · `10.5`·`11`·`12px`→`xs`(11.813) · `13px`→`sm`(12.938). `10.5px` 만 거리로는 `badge` 가 가깝지만 **의도를 따라 `xs`** 로 보냈다 — 값이 `xs` 의 base-intent(10.5px)와 정확히 일치해 DESIGN.md 표의 base-intent 열을 px 로 박은 흔적이고, 용도(섹션 라벨·열 헤더)도 `xs` 의 "Small-caps labels" 와 맞다. **크기 변화는 이 작업의 부작용이 아니라 목적**이다(최대 `10.5px`→11.813px, +12.5%; 최다인 `10px`→9.563px 는 −4.4%). `badge` 의 usage 를 "SymbolSearch market tag" 에서 "micro-labels(상태 칩·마커·카운트 배지·단축키 표기)" 로 넓혔다. **검증은 다이얼을 실제로 돌려서 했다** — `:root` 18px→20px 에서 네 토큰(9.563/11.813/12.938/14.625)이 전부 정확히 ×1.111 로 따라오고 개수도 불변, 즉 낙오된 px 가 0. 5개 라우트에서 클리핑·가로스크롤 0건. |
