...
- **Themes (three, selectable):** the app ships three full palettes behind `<html data-theme>`:
  ...
  - **Toss Light** (light, `toss-light`) — white cards on a grey floor + toss-blue accent. Benchmarked from tossinvest.com's live tokens (2026-07-22). **Manual-select only** — `auto` never resolves to it. See the Toss Light note below the token table for the accent-vs-price collision it carries.
  - The **preference** (`obsidian` / `ledger` / `toss-light` / `auto`) lives in `state/themePrefs.ts` (localStorage `ui.themePreference.v1`); `auto` maps `/live` + `/heatmap` → Obsidian and everything else → Ledger via `effectiveTheme(pref, pathname)` (auto only ever picks Obsidian or Ledger — never Toss Light). `App.tsx` writes the resolved theme to `data-theme`; `index.html` sets it inline before first paint (FOUC + wrong-theme chart cache guard).
  - **Selectors:** `:root, [data-theme='dark'], [data-theme='obsidian']` carry the Obsidian palette + the scale dial + font + all size/spacing/layout tokens; `[data-theme='ledger']` and `[data-theme='toss-light']` each override **colors only** (density and typography are theme-independent). Both light blocks sit *outside* the base block so `npm run gen:tokens` never touches them.

  > Table below shows the two primary themes (Obsidian/Ledger). **Toss Light** is a
  > third light palette; rather than widen every row, its values live only in
  > `tokens.css` `[data-theme='toss-light']` and the summary note directly under the table.
...
- **No OS/system mode:** there is no "system/OS" auto-follow mode. `data-theme` is exactly `obsidian`, `ledger`, or `toss-light` at runtime (the legacy `dark` alias maps to Obsidian). `auto` is a *route*-based preference (dark on `/live`·`/heatmap`, else Ledger), not an OS-preference follow, and it never resolves to `toss-light` — Toss Light is manual-select only. (History: this was "both themes only, no third theme" until 2026-07-22, when Toss Light was added as an explicitly manual third palette.)
...
- **Responsive floor (`--app-floor-min-w`, 2026-07-21):** the app has **one floor for every route**: `57rem` (1026px @ default density, 912px base intent). Above it the shell compresses fluidly; below it the shell stops compressing and `#root` scrolls horizontally (`global.css`). There is **no zoom-detection code anywhere** — browser zoom *is* effective-viewport narrowing, so the width response covers zoom for free (`visualViewport.scale` branching is prohibited: it double-counts window resize vs zoom and doubles the test matrix).
  - **Why a single global value:** the floor is set by the **shell chrome every route shares**, not by page content. Measured 2026-07-21 at default density: TopNav natural width 939px + `--rail-w` 54px = **993px**; the token adds 33px headroom. Content-light routes (`/settings`, `/capture`) would reflow narrower, but the chrome breaks first, so per-page floors add complexity with no gain. `App.tsx` wraps every route via `<Outlet/>`, so one declaration reaches all pages and a new route can't forget it.
  ...
  - **Re-deriving the floor:** measure the chrome, don't guess. `document.querySelector('nav').firstElementChild` → set `width:max-content` → read `getBoundingClientRect().width`, add the rail. Bump the token when the nav gains items.
