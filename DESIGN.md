...
- **Themes (dual, selectable):** the app ships two full palettes behind `<html data-theme>`:
  ...
  |---|---|---|---|
  | `--bg` | `#121216` | `#FDFCF8` | App background — 양 테마 모두 `--bg-card` 와 동일 톤(배경 통일, 2026-07-15 #636 다크 · #637 라이트). Obsidian 분리=4px gap+`--shadow-panel`; Ledger 분리=카드 `--border`(feature-route 카드는 테두리 유지) |
...
- **Content framing:** primary content sits in `bg-bg-card border rounded-lg` cards. Multi-pane pages (master-detail, splitter) use one card per pane; single-content pages use one card. Never nest cards. **Obsidian-only exception (`/heatmap`, 2026-07-15):** `/heatmap` is an Obsidian-only surface (`OBSIDIAN_ROUTE_PREFIXES`) where `--bg`=`--bg-card`, so its single `PanelCard` uses `borderless` (`PanelCard borderless` prop) to sit on `--bg` exactly like the `/live` chart panel — 테두리 없이 `--bg-card` + `--shadow-panel` 만으로 분리. Its inner header/섹터 스트립 밴드는 `bg-subtle`→`bg-card` 로 평탄화하고 구분선은 `border-strong`→`border` 로 완화(live `WorkspaceToolbar` = `bg-card` + `border-b border-border` 와 동형). The border-retention rule below still governs **Ledger** feature routes.
