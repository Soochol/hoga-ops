...
- **Themes (dual, selectable):** the app ships two full palettes behind `<html data-theme>`:
  ...
  | `--bg-subtle` | `#0E0E11` | `#F2EFE7` | TopNav·드로어(RailShell)·설정/지표 nav 크롬, dropdown/sticky headers, recessed rows (`/study` 워크스페이스 탭 바·`/live` price strip 은 `--bg` 로 통일 — 여기서 제외; `/live` 멀티 탭은 ADR-0113 으로 제거됨) |
