...
### Workspace window — 빈/미지원 상태 (`/live`)

`live/workspace/WindowEmptyState.tsx` 가 차트 창·데이터 창의 빈 상태를 함께 소유한다.

- **전면 오버레이 금지.** 창은 자유 배치라 각자 자기 그룹의 상태를 말한다 — 워크스페이스
  한가운데 "종목을 선택하세요" 를 띄우면 창 독립성이 깨진다(`tests/e2e/live-smoke.spec.ts`
  가 이 배치 계약을 고정한다).
- **상태 + 다음 행동 2단.** 제목(`xs`/500/`--fg`)은 무엇이 비었는지(`10호가 · 종목 없음`),
  힌트(`11px`/`--fg-dim`)는 무엇을 하면 채워지는지. **상태만 쓰지 말 것** — 하단 바에
  "실시간 N종목" 이 흐르는 화면에서 상태만 보이면 사용자가 정상 연결과 고장을 구분할 수
  없다(2026-08-04 이전이 그랬다).
- **주 표면만 온전한 안내를 진다.** 차트 창 = 그룹의 주 표면이라 진입로까지 말하고
  (`검색(/) 이나 우측 목록에서…`), 데이터 창은 축약판(`그룹 N 에 종목을 지정하면
  표시됩니다`)이다. 창 수만큼 같은 문장을 반복하지 않기 위한 배분이다.
- **할 수 있는 일이 없으면 `hint` 를 사유로 쓴다** — 지수 그룹에 10호가 창을 띄운 경우처럼
  사용자의 선택이 원인이면 "지수는 지원하지 않습니다" 로 끝낸다(시킬 행동이 없다).
- 페이지 카드용 `ui/DataSurface` 의 `EmptyState`(`p-lg`/`text-base`)와 **다른 컴포넌트**다.
  워크스페이스 창은 사용자가 임의로 줄일 수 있어 한 단계 낮은 타이포를 쓴다.
