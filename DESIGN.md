...
- **Themes (four, selectable):** the app ships four full palettes behind `<html data-theme>`:
  ...
  - **Toss Light** (light, `toss-light`) — white cards on a grey floor + toss-blue accent. Benchmarked from tossinvest.com's live tokens (2026-07-22). **Manual-select only** — `auto` never resolves to it. See the Toss Light/Dark note below the token table for the accent-vs-price collision it carries.
...
- **Sidebar width:** 320px base intent / 360px rendered (token: `--sidebar-w`). 현재 유일한 소비처는 `/inventory` 의 master-detail 좌열(`pages/Inventory.tsx`) — 토큰 주석이 말하는 "replay viewer 의 Cursor Sidebar" 는 존재하지 않는다(2026-07-29 실측).
...
**Tokens defined but no longer consumed** (실측 2026-07-29, `frontend/src` 전수 grep — `tokens.css`/`design-tokens.ts`/`tokens.generated.ts` 정의 파일 제외): `--h-pricestrip`(시세 스트립 폐지), `--h-tab`·`--h-tab-secondary`, `--h-orderbook-row`, `--combobox-min-w`. 정의는 남아 있으므로 **여기 있는 값이 화면에 나타난다고 읽지 말 것** — 쓰려면 소비처를 먼저 만들어야 한다. 같은 이유로 `design-tokens.ts` 의 `usage` 문자열 일부도 페이지 이름이 낡아 있다(`--h-toolbar` "Replay-page toolbar row", `--sidebar-w` "Cursor sidebar on the right of replay viewer") — 이 문자열은 `npm run gen:tokens` 로 `tokens.css` 주석에 그대로 찍히므로, 고칠 때는 `design-tokens.ts` 를 고치고 재생성해야 한다.
