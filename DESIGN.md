...
| `--h-live-header` | 32px | 32px | ⚠ **소비처 0** (2026-08-23, ADR-0157). 마지막 소비자 `WorkspaceShell.WorkspaceHeader` 가 `/study` 와 함께 삭제됐다. 그마저 호출부가 `min-h-12`(3rem = 48px @ 1×) 를 얹고 있었으므로 **이 토큰은 한 번도 화면 높이를 정한 적이 없다**(`min-height` 가 인라인 `height` 를 이긴다 — #900 의 "원인은 버튼이 아니라 2줄 식별부"). 제거는 디자인 시스템 패스에서 판단할 것 |
